% seqMemTask_Curricula_Traj_anal.m
% write by XR @ April 3 2026
% Trajectory analysis for each retrieval test

% ---------- Description ----------
% YA (18-35 yrs) and OA (65-75 yrs)
% Three sub-groups within each age group:
% Interleaved, ObjectBlocked, LocationBlocked

clear
clc

%%
addpath('tight_subplot/');
addpath(genpath('HierarchicalCluster/'));
addpath(genpath('Aging-SeqMemTask/'));
% addpath(genpath('Violinplot-Matlab-master/'));
addpath('fdr_bh');

%% parameters
folder     = '/Users/ren/Projects-NeuroCode/MyExperiment/Aging-SeqMemTask';
bhvDataDir = [folder, '/AgingReplay-OnlineData'];
CLdata_folder = [bhvDataDir, '/CurriculumPaper-Data/']; % data for the summary of the curriculum learning

RChunk = 0.35;
nSes    = 8*2; % each block contain 4 sequences, the 4 unique sequences will be repeated 8 times in 8 blocks
nImgSeq = 2; % 2 unique content sequence
nPosSeq = 2; % 2 unique position sequence
nPos    = 8; % 8 positions uniformly distributed on a circle
nImg    = 8; % 8 unique images
nTrans  = 5; % each sequence contains 5 transitions; 5 transitions = 5 categories
nEpi    = nImgSeq * nPosSeq * nSes;
nDtr    = 1; % number of distractor
postTn  = 8; % trials of post-testing
trlBlc  = 8; % each block has 8 trials
uniTrl  = 4; % 4 unique trials
nBlock  = nEpi / trlBlc; % 8 blocks
nComb   = 4; % 4 unique combinations between 2 content sequence and 2 position sequence
angCir = 0 : pi/50 : 2 * pi;
centerX = 0;
centerY = 0;
xCir   = RChunk * cos(angCir) + centerX;
yCir   = RChunk * sin(angCir) + centerY;

expList = {'interleaved', 'contentBlocked', 'positionBlocked'};
nCond   = length(expList);
expId   = expList{3};
if isequal(expId, 'interleaved')
    subjList_young = {'5ad63c167f70c10001904bc5', '2023-08-30_17h17.39.428'; '5bdb51e1ba9b510001052364', '2023-08-30_15h12.00.151'; '5c4b06903566570001309394', '2023-08-30_16h55.13.543'; ...
                      '5d024a1fb58b6f001a58f74d', '2023-08-30_15h11.44.361'; '5d43404f1e6eef00011dec22', '2023-08-30_15h12.02.990'; '5ef25afb8ebcdf0b2b95d9cd', '2023-08-30_15h09.37.394'; ...
                      '5f15f96e54587538da27d452', '2023-08-30_15h40.43.668'; '5fd0c81fc79aef1882cbee94', '2023-08-30_16h25.12.136'; '60fecc838b1c231b1732cbb0', '2023-08-30_15h07.34.541'; ...
                      '601f93758d79b24eabff2e44', '2023-08-30_15h11.55.950'; '602fc5844525b3d343303a2a', '2023-08-30_14h05.56.283'; '604be8ac8e0c517878fd1d9f', '2023-08-30_14h05.21.886'; ...
                      '612ecc90331b627f7aaac5dc', '2023-08-30_16h23.19.001'; '614fca831894ddce32c1a342', '2023-08-30_15h20.32.432'; '615b5902e51bcad574d81203', '2023-08-30_15h18.25.454'; ...
                      '6016c8e7ea3f2387ae8b47d5', '2023-08-30_16h11.53.139'; '6103c08d411c6be73d9d78a7', '2023-08-30_15h20.28.394'; '6159f6b637bab134ea9bb92e', '2023-08-30_15h13.34.934'; ...
                      '61070b50a022d7360e46e985', '2023-08-30_16h08.26.937'; '61353c933f32fef782432cc7', '2023-08-30_15h10.45.788'; '605272be8568b6160f582f2e', '2023-08-30_14h38.14.393'; ...
                      '6107292e60892e4246db7425', '2023-08-30_15h12.11.729'; '61685478a9bd5239a9438f66', '2023-08-30_15h12.25.867'; '614831813dc412ccc8e2f563', '2023-08-30_15h31.25.179'};

    subjList_old   = {'5abb8dcb7ccedb0001b7f0d7', '2023-05-23_15h56.51.831'; '5be064114c6bd000013368f3', '2023-05-22_18h00.20.474'; '5c5df0475b87820001c4f21c', '2023-05-23_16h15.47.308'; ...
                      '5e9f0bc126557006ea49d1f4', '2023-05-23_16h58.16.506'; '5ea20fd571038c119083a8df', '2023-05-23_15h56.44.482'; '63b2d04ed0f53f75de4ba38e', '2023-05-23_16h30.06.501'; ...
                      '609a503448860549084c43ce', '2023-05-22_17h13.56.435'; '60534c39d754d351333bdd7c', '2023-05-23_16h15.50.361'; '597519f8262c480001bbaf8b', '2023-05-23_17h49.49.000'; ...
                      '61539b3fa541b182c0fadde1', '2023-05-26_11h37.17.532'; '574ce0a57fd0ec000db73aa6', '2023-05-26_12h33.38.572'; '55900dcffdf99b3f7aada3f5', '2023-05-26_10h08.56.439'; ...
                      '55e9aa1c735c45001043fbb6', '2023-05-26_17h56.53.674'; '64456ad3d3e7651a1dad232c', '2023-05-26_11h53.36.716'; '62aa26dd93252c8d69f7fc45', '2023-05-26_17h51.56.081'; ...
                      '5f53b958c8cfea6e2104c5b6', '2023-05-26_17h23.34.078'; '5f48e3d7f998433ac6356ad4', '2023-05-26_11h52.29.081'; '62f0f033178f89dd6f416590', '2023-05-26_17h21.29.494'; ...
                      '5c79a584670f87001646cef6', '2023-05-26_17h42.42.514'; '630be3605287a0f49b87c709', '2023-05-26_16h37.54.676'; '6121190671d1042b24d8d67b', '2023-05-26_16h16.28.267'; ...
                      '5c4cdcb14cb4630001ec4955', '2023-05-26_16h17.13.674'; '5f6e83419dd5cb3c85325fc6', '05-26-2023_16h37.34.939'}; % '63beebaa4c5884797ff00a98', '2023-05-26_17h39.52.151': attend contentBlocked

    ageList_younger    = [24, 22, 23, ...
                          25, 24, 21, ...
                          22, 23, 28, ...
                          21, 22, 23, ...
                          24, 30, 28, ...
                          27, 22, 24, ...
                          21, 20, 26, ...
                          26, 33, 28];
    genderList_younger = {'F', 'M', 'F', ...
                          'M', 'F', 'M', ... % the 4th participant: male(trans)
                          'M', 'M', 'F', ...
                          'M', 'F', 'M', ...
                          'M', 'M', 'F', ...
                          'F', 'F', 'F', ...
                          'F', 'M', 'F', ...
                          'M', 'F', 'M'}; 

    ageList_older    = [67, 68, 67, ...
                        69, 65, 66, ... % 5th participant: 64???
                        73, 73, 70, ...
                        65, 72, 72, ...
                        65, 69, 67, ...
                        67, 65, 68, ...
                        70, 72, 65, ...
                        67, 67];
    genderList_older = {'F', 'F', 'M', ...
                        'M', 'F', 'F', ...
                        'F', 'M', 'F', ...
                        'F', 'F', 'M', ...
                        'F', 'F', 'M', ...
                        'M', 'F', 'F', ...
                        'F', 'F', 'M', ...
                        'M', 'F'};

elseif isequal(expId, 'contentBlocked')
    subjList_young = {'5a6e4ecae6cc4a0001b6d38d', '2023-08-30_14h14.47.654'; '5bcdb05e1bfcbf0001d77240', '2023-08-30_15h43.03.664'; '5eceef5fa487421604c337ba', '2023-08-30_19h06.46.141'; ...
                      '5f1f1a1f443fd90bf5e2e716', '2023-08-30_15h12.16.243'; '5f4fd62570b0df0f71a35d98', '2023-08-30_16h34.02.959'; '5f5f6e9b003b2a0217bba847', '2023-08-30_16h33.19.240'; ...
                      '5f8825d4938a85280f506a83', '2023-08-30_14h12.31.867'; '60db9c9850c39eea109ef1d3', '2023-08-30_15h14.10.340'; '60f31ca80f6c233558e5a354', '2023-08-30_15h14.19.785'; ...
                      '603e2530ab9d37d734fa6ca9', '2023-08-30_15h20.00.625'; '610a5f883d6841e65838f97d', '2023-08-30_15h20.15.081'; '611d604624f673b1e62275c5', '2023-08-30_15h14.05.541'; ...
                      '612cf0efe0be33cea5c5a123', '2023-08-30_15h13.46.115'; '615cc500aab10659f82a02ab', '2023-08-30_15h20.16.812'; '616fd6aac8d209bdcd631c2a', '2023-08-30_15h15.09.944'; ...
                      '6106e9f1880fb0b44c319ced', '2023-08-30_14h13.50.009'; '6130e97d4106299f8c6120fa', '2023-08-30_15h13.59.129'; '6151e74c66fb9fb95b2f522e', '2023-08-30_15h17.08.514'; ...
                      '6159bec91e6d099cb2b032fc', '2023-08-30_15h14.49.228'; '60561bed5ea5ad8dbe3fae07', '2023-08-30_14h38.38.277'; '61698b3f8623f619b602b00b', '2023-08-30_15h07.19.062'; ...
                      '610063b7b50c4e9488e77eca', '2023-08-30_15h16.38.986'; '617091df73f7dd1c8448b3f4', '2023-08-30_15h14.45.308'; '615024818c0798f950215d49', '2023-08-30_15h13.03.877'};

    subjList_old   = {'5c964575c7f75b000167754e', '2023-05-23_16h14.31.968'; '5dfb7cbd01423f8a774d893b', '2023-05-23_18h31.24.688'; '5e8f569436e20a234f89a6f4', '2023-05-23_16h13.25.490'; ...
                      '5ea0b2cbf710490ac2644b7e', '2023-05-23_16h20.30.392'; '5ea3319a6a1a5b2a1175ed6e', '2023-05-23_16h15.41.365'; '5ea159434ac916016387488e', '2023-05-23_15h58.15.599'; ...
                      '58e79d86fe9c8c0001c77ced', '2023-05-23_16h11.20.170'; '574da26c7f1e770007f42d11', '2023-05-23_16h16.02.150'; '614c5dc3cda534db7afc2e73', '2023-05-23_16h09.12.297'; ...
                      '614f874e5b46971822dfa61a', '2023-05-23_18h55.58.563'; '6161bdbff67e4b4621b530e7', '2023-05-23_17h11.58.970'; '59eb2cc98c371000010bb196', '2023-05-26_18h01.10.451'; ...
                      '5e86c11942701c2ffda5d113', '2023-05-26_11h48.52.812'; '6086c333d6eb73cfdd564e90', '2023-05-26_17h01.07.280'; '5eb2695f1745801c7c919e35', '2023-05-26_16h36.21.594'; ...
                      '5af32f9d003f6c0001f2905b', '2023-05-26_16h27.35.772'; '61703be3748d6f5ddc01170a', '2023-05-26_17h11.26.871'; '610c67785d74ee2c4a39def8', '2023-05-26_16h24.47.208'; ...
                      '5c8ee6c36ca70b0001fe979d', '2023-05-26_11h13.31.213'; '63beebaa4c5884797ff00a98', '2023-05-26_16h33.19.754'; '62162ab683fc823e78c025e5', '2023-05-26_16h09.16.187'; ...
                      '5ab14bdeb0ca80000197e6b6', '2023-05-26_16h15.01.130'; '6452058d0baefbe199f321e0', '2023-05-26_15h56.07.802'; '5e510d0760dd0913e45370dc', '2023-05-26_15h56.04.808'; ...
                      '5c081c45fd9c080001709937', '2023-05-26_15h51.35.355'};

elseif isequal(expId, 'positionBlocked')
    subjList_young = {'5eac7f2a11f5972d923bcd8e', '2023-08-30_15h14.56.423'; '5ecfdd84dc64e1061b97e321', '2023-08-30_15h16.52.236'; '5f82fd997dab234303560326', '2023-08-30_16h05.14.338'; ...
                      '60aadeb9e6e8147089f7eced', '2023-08-30_15h10.48.242'; '60cca032f398af85575618e3', '2023-08-30_15h14.23.924'; '60d333a37d135f2ee2592457', '2023-08-30_14h06.57.777'; ...
                      '60f5db51ea1f75902fc20970', '2023-08-30_14h26.24.581'; '60f6a19c247160dce8d5a69c', '2023-08-30_17h45.57.432'; '60fb0d1ef1ea8d2bcb8166dd', '2023-08-30_16h02.38.876'; ...
                      '64c12183ab9cf635c69df81b', '2023-08-30_15h10.11.992'; '603e5d265ed1c2e3ea13ebad', '2023-08-30_15h12.42.164'; '611b87ab5cc971129768ead2', '2023-08-30_15h16.28.053'; ...
                      '611d06c0bcc92ba3d7669ef6', '2023-08-30_15h20.29.244'; '611e60a6a1fd59a57341b862', '2023-08-30_15h18.11.988'; '60940b7855b3a885f925856b', '2023-08-30_14h06.05.080'; ...
                      '61544c72236c88d054490ea6', '2023-08-30_15h06.40.921'; '64736ec17f1a9b745c8fad92', '2023-08-30_14h24.33.592'; '613615da1eacf6204ce33479', '2023-08-30_14h05.15.755'; ...
                      '64526929d8f9b780b29d4d8d', '2023-08-30_16h31.49.928'; '6175733727b1e3ce2d72dbe4', '2023-08-30_15h24.49.630'; '61412724735027d42bf53011', '2023-08-30_15h12.53.447'};

    subjList_old   = {'5a9e9fc46219a30001f54994', '2023-05-23_17h08.22.534'; '5ab8d182e1546900019b7195', '2023-05-23_16h22.50.058'; '5b017ef1293d310001023bd8', '2023-05-23_16h11.23.900'; ...
                      '5d812e3c613aa900188746a6', '2023-05-23_16h31.34.361'; '60ce6af707bcd42cbc885210', '2023-05-23_16h30.17.733'; '60f34fcae3c49524b0903a5d', '2023-05-23_16h19.52.799'; ...
                      '60f728553a37102574b585c4', '2023-05-23_11h34.06.217'; '612cc22830e71399b7a86841', '2023-05-23_16h23.00.515'; '6130a32cd30a251765045601', '2023-05-23_16h57.41.603'; ...
                      '64457bc906c125cebd4bf66b', '2023-05-23_16h49.30.176'; '608e2cb9067eb028500433d5', '2023-05-26_12h31.32.235'; '60c119b30aa5205b493541b6', '2023-05-26_14h42.15.132'; ...
                      '64071f8576c48034c00df845', '2023-05-27_01h35.27.844'; '5b33a01fa8327d0001003821', '2023-05-26_13h12.16.403'; '5f9ec66a5a97fa0748bc61a3', '2023-05-26_12h09.26.191'; ...
                      '6148c0a6e2353cbbac1cd506', '2023-05-26_10h08.10.469'; '597e0aa515837000016ae8db', '2023-05-26_17h13.06.495'; '5e9027110aacc7320bd9a84b', '2023-05-26_17h49.02.839'; ...
                      '5b4e50fb369f840001136070', '2023-05-26_17h21.53.119'; '558bb476fdf99b21155f2dbf', '2023-05-26_17h05.08.565'; '5e54367e80cd0944205b27f9', '2023-05-26_17h00.55.134'; ...
                      '57dc590ddcda780001a0e157', '2023-05-26_17h35.13.259'; '612fa816410c4ea2f08fe22c', '2023-05-26_17h21.19.703'; '5c28b31a0091e40001ca5030', '2023-05-26_17h05.03.096'; ...
                      '5be92cf1ba2782000117743e', '2023-05-26_17h00.40.846'}; %% '622a29743b7c0ca5eee56e24', '2023-05-23_16h04.13.134': this participant did nothing in reconstruction report

end
nGroup = 2; %% younger and older adults
nSub_group = nan(2, 3); % 2: YA and OA; 3: interleaved/contentBlocked/positionBlocked
nSub_group(:, 1) = [24, 23]';
nSub_group(:, 2) = [24, 25]';
nSub_group(:, 3) = [21, 25]';

%% Extracting the mouse trajectory and computing distance to each stimulus
% For each retrieval trial and each time sample in the trajectory,
% compute the Euclidean distance from the mouse to every on-screen stimulus.
%
% Content report  : 6 images in a fixed horizontal row (imgDispX / imgDispY,
%                   saved once per session in the genStim row).
% Location report : 6 circle slots (5 targets + 1 distractor) that vary per
%                   trial (locSeqXTrl / locSeqYTrl, in each episodeStart row).
% Both report     : 6 draggable images at inner-circle positions
%                   (bothConPosXTrl / bothConPosYTrl) + 6 outer-circle empty
%                   slots (locSeqXTrl / locSeqYTrl).
%
% Output per subject:
%   mouse_*.trajectory  – [nT × 3]  (x, y, t) samples during the report
%   mouse_*.stimX/Y     – [1 × 6]   on-screen positions of the 6 stimuli
%   mouse_*.slotX/Y     – [1 × 6]   slot positions (both report only)
%   mouse_*.distToStim  – [nT × 6]  distance to each stimulus at each sample
%   mouse_*.distToSlot  – [nT × 6]  distance to each slot   (both only)
%   mouse_*.trialNo     – scalar     0-based trial index from the JS

suffixWord = expId;

% Group-level containers: all_mouse_*_grp{iGrp}{iSub} = struct array of trials
all_mouse_con_grp  = cell(nGroup, 1);
all_mouse_loc_grp  = cell(nGroup, 1);
all_mouse_both_grp = cell(nGroup, 1);

for iGrp = 1 : nGroup
    if iGrp == 1
        groupName = 'younger';
        subj_list = subjList_young;
    elseif iGrp == 2
        groupName = 'older';
        subj_list = subjList_old;
    end
    subLen   = size(subj_list, 1);
    subjPath = [bhvDataDir, '/AgingReplay-v2-', suffixWord, '/', suffixWord, '-', groupName, '/'];

    all_mouse_con  = cell(subLen, 1);
    all_mouse_loc  = cell(subLen, 1);
    all_mouse_both = cell(subLen, 1);

    for iSub = 1 : subLen
        %%
        subjBv = subj_list{iSub, 1};
        subjTm = subj_list{iSub, 2};

        % ------ Read the csv file ------
        filename = [subjPath, subjBv, '_EpisodicMemoryTask-', suffixWord, '_', subjTm, '.csv'];

        opts = detectImportOptions(filename, ...
                'Delimiter', ',', ...
                'VariableNamingRule', 'preserve');
        opts = setvartype(opts, opts.VariableNames, 'string');
        seqMem_subj = readtable(filename, opts);

        % ------ Extract mouse trajectories for each retrieval type ------
        mouse_con  = extractMouseClosestToReport(seqMem_subj, 'mouseX',     'mouseY',     'mouseT',     'conReportTrue');
        mouse_loc  = extractMouseClosestToReport(seqMem_subj, 'mouseXloc',  'mouseYloc',  'mouseTloc',  'locReportTrue');
        mouse_both = extractMouseClosestToReport(seqMem_subj, 'mouseXboth', 'mouseYboth', 'mouseTboth', 'bothReportTrue');

        fprintf('[%s | Sub %02d]  con: %d   loc: %d   both: %d\n', ...
            groupName, iSub, length(mouse_con), length(mouse_loc), length(mouse_both));

        % ------ Compute content-report image positions from JS parameters ------
        % imgDispX/Y are determined by three constants hardcoded in the JS
        % (imgSize=0.15, nTrans=5, nDtr=1) and are identical for every
        % participant and trial, so we derive them directly rather than
        % reading from the CSV (genStim runs outside any TrialHandler loop,
        % so the column may never appear in the CSV output).
        imgSize_js      = 0.15;
        imgStart        = -imgSize_js * nTrans / 2;
        imgEnd          =  imgSize_js * nTrans / 2;
        iX_vec          = 0 : (nTrans + nDtr - 1);
        imgDispX_global = (imgStart - imgSize_js) + ...
                          (imgEnd + imgSize_js - (imgStart - imgSize_js)) / nTrans .* iX_vec; % [1×6]
        imgDispY_global = repmat(imgSize_js, 1, nTrans + nDtr); % [1×6], all 0.15

        % ------ Locate episode-start rows (per-trial position data lives here) ------
        % trialNo is saved only in the episodeStart routine, once per formal trial.
        epi_rows = find(~ismissing(seqMem_subj.('trialNo')) & ...
                        strlength(seqMem_subj.('trialNo')) > 0);

        % ------ Attach on-screen stimulus positions to each trajectory trial ------
        mouse_con  = addStimPos(mouse_con,  'con',  seqMem_subj, epi_rows, imgDispX_global, imgDispY_global);
        mouse_loc  = addStimPos(mouse_loc,  'loc',  seqMem_subj, epi_rows, [], []);
        mouse_both = addStimPos(mouse_both, 'both', seqMem_subj, epi_rows, [], []);

        % ------ Compute Euclidean distance from each trajectory sample to each stimulus ------
        mouse_con  = computeDistToStim(mouse_con);
        mouse_loc  = computeDistToStim(mouse_loc);
        mouse_both = computeDistToStim(mouse_both);

        % ------ Analyze mind-changing before each sequential choice ------
        mouse_con  = analyzeChoiceHesitation(mouse_con,  seqMem_subj, 'con',  nTrans, nDtr);
        mouse_loc  = analyzeChoiceHesitation(mouse_loc,  seqMem_subj, 'loc',  nTrans, nDtr);
        mouse_both = analyzeChoiceHesitation(mouse_both, seqMem_subj, 'both', nTrans, nDtr);

        % Collect per-subject results
        all_mouse_con{iSub}  = mouse_con;
        all_mouse_loc{iSub}  = mouse_loc;
        all_mouse_both{iSub} = mouse_both;
    end

    all_mouse_con_grp{iGrp}  = all_mouse_con;
    all_mouse_loc_grp{iGrp}  = all_mouse_loc;
    all_mouse_both_grp{iGrp} = all_mouse_both;
end
%% save the trajectory data for subsequent statistical test prepration
% save([CLdata_folder, 'all_mouse_con_grp_YAOA_', suffixWord, '.mat'], 'all_mouse_con_grp');
% save([CLdata_folder, 'all_mouse_loc_grp_YAOA_', suffixWord, '.mat'], 'all_mouse_loc_grp');
% save([CLdata_folder, 'all_mouse_both_grp_YAOA_', suffixWord, '.mat'], 'all_mouse_both_grp');

%% Prepare data for subsequent statistical tests
% only for the average metric per trial
condsList = {'interleaved', 'contentBlocked', 'positionBlocked'};
trajCell_allConds = [];
for iConds = 1 : length(condsList)
    conds_ii = condsList{iConds};
    % ------ content ------
    load([CLdata_folder, 'all_mouse_con_grp_YAOA_', conds_ii, '.mat']);
    all_mouse_con_iConds  = all_mouse_con_grp;
    % ------ location ------
    load([CLdata_folder, 'all_mouse_loc_grp_YAOA_', conds_ii, '.mat']);
    all_mouse_loc_iConds  = all_mouse_loc_grp;
    % ------ both ------
    load([CLdata_folder, 'all_mouse_both_grp_YAOA_', conds_ii, '.mat']);
    all_mouse_both_iConds = all_mouse_both_grp;

    % ------ Reorganize data ------
    for iGrp = 1 : nGroup
        % ------ content ------
        all_mouse_con_iGrp  = all_mouse_con_iConds{iGrp};
        % ------ location ------
        all_mouse_loc_iGrp  = all_mouse_loc_iConds{iGrp};
        % ------ both ------
        all_mouse_both_iGrp = all_mouse_both_iConds{iGrp};
        subLen_iConds = length(all_mouse_con_iGrp);
        for iSub = 1 : subLen_iConds
            all_mouse_con_iSub  = all_mouse_con_iGrp{iSub}; % a structure
            all_mouse_loc_iSub  = all_mouse_loc_iGrp{iSub}; 
            all_mouse_both_iSub = all_mouse_both_iGrp{iSub};  

            trajMetrics_con  = nan(size(all_mouse_con_iSub, 1), 2 * 3); % 2: correct vs. incorrect; 3: three types of trajectory-related metrics
            trajMetrics_loc  = nan(size(all_mouse_con_iSub, 1), 2 * 3); 
            trajMetrics_both = nan(size(all_mouse_con_iSub, 1), 2 * 3); % not include the post-test trials
            % -- concatenate the correct and incorrect responses of the trajectory-related metrics --
            for iTrl = 1 : size(all_mouse_con_iSub, 1) % only the learning-retrieval trials
                % ---- object retrieval ----
                trajMetrics_con(iTrl, :) = [all_mouse_con_iSub(iTrl).interClickTime_corr, ...
                                            all_mouse_con_iSub(iTrl).interClickTime_incorr, ...
                                            all_mouse_con_iSub(iTrl).selSpeed_corr, ...
                                            all_mouse_con_iSub(iTrl).selSpeed_incorr, ...
                                            all_mouse_con_iSub(iTrl).dwellTime_corr, ...
                                            all_mouse_con_iSub(iTrl).dwellTime_incorr];
                % ---- location retrieval ----
                trajMetrics_loc(iTrl, :) = [all_mouse_loc_iSub(iTrl).interClickTime_corr, ...
                                            all_mouse_loc_iSub(iTrl).interClickTime_incorr, ...
                                            all_mouse_loc_iSub(iTrl).selSpeed_corr, ...
                                            all_mouse_loc_iSub(iTrl).selSpeed_incorr, ...
                                            all_mouse_loc_iSub(iTrl).dwellTime_corr, ...
                                            all_mouse_loc_iSub(iTrl).dwellTime_incorr];

                % ---- full retrieval ----
                trajMetrics_both(iTrl, :) = [all_mouse_both_iSub(iTrl).interClickTime_corr, ...
                                             all_mouse_both_iSub(iTrl).interClickTime_incorr, ...
                                             all_mouse_both_iSub(iTrl).selSpeed_corr, ...
                                             all_mouse_both_iSub(iTrl).selSpeed_incorr, ...
                                             all_mouse_both_iSub(iTrl).dwellTime_corr, ...
                                             all_mouse_both_iSub(iTrl).dwellTime_incorr];

            end
            trajMetrics_col = [reshape(trajMetrics_con, size(trajMetrics_con, 1) * size(trajMetrics_con, 2), 1); ...
                               reshape(trajMetrics_loc, size(trajMetrics_loc, 1) * size(trajMetrics_loc, 2), 1); ...
                               reshape(trajMetrics_both, size(trajMetrics_both, 1) * size(trajMetrics_both, 2), 1)];

            % -- conditions --
            iConds_col = repmat(iConds, length(trajMetrics_col), 1);

            % -- age group --
            iGrp_col   = repmat(iGrp, length(trajMetrics_col), 1); % 1-YA; 2-OA

            % -- subj --
            iSub_col   = repmat(iSub, length(trajMetrics_col), 1);

            % -- trial labels --
            trial_idx  = (1 : 1 : size(all_mouse_con_iSub, 1))';
            trial_col  = repmat(trial_idx, 2 * 3 * 3, 1); % the 2nd 3: object/location/full retrieval

            % -- retrieval types: object/location/full --
            object_col    = ones(size(all_mouse_con_iSub, 1) * 2 * 3, 1);
            location_col  = repmat(2, size(all_mouse_con_iSub, 1) * 2 * 3, 1);
            full_col      = repmat(3, size(all_mouse_con_iSub, 1) * 2 * 3, 1);
            retrieval_col = [object_col; location_col; full_col];

            % -- trajectory-related confidence metrics labels --
            interclick_col  = ones(size(all_mouse_con_iSub, 1) * 2, 1); % 2: correct and incorrect
            seletionSpd_col = repmat(2, size(all_mouse_con_iSub, 1) * 2, 1);
            dwellTime_col   = repmat(3, size(all_mouse_con_iSub, 1) * 2, 1);
            trajMet_label_col = repmat([interclick_col; seletionSpd_col; dwellTime_col], ...
                                        3, 1); % 3: retrieval types - object/location/full

            % -- correct or incorrect response labels --
            cor_incor_col = repmat([ones(size(all_mouse_con_iSub, 1), 1); zeros(size(all_mouse_con_iSub, 1), 1)], ...
                                   3 * 3, 1); % 1-correct; 0-incorrect

            trajCell_allConds = [trajCell_allConds; ...
                                 iConds_col, iGrp_col, iSub_col, trial_col, ...
                                 retrieval_col, cor_incor_col, ...
                                 trajMet_label_col, trajMetrics_col];
        end
    end
end
%% ------ save the trajectory data for subsequent statistical tests ------
% save([CLdata_folder, 'trajCell_allConds_YAOA.mat'], 'trajCell_allConds');

%% Consistency check: trajectory-inferred pick-up vs response data (Full report)
% Prints a per-group summary table and plots match rate by sequential position.
% The consistency figure now shows pick-up detection rate (how often participants' cursor passes through the inner zone before each drag), 
% which is actually more scientifically useful than the broken image-identity cross-check. 
% A rate well below 100% at a particular position would indicate the participant sometimes dragged without hovering at an inner image first.

fprintf('\n--- Pick-up consistency check (both report) ---\n');
fprintf('%-12s  %6s  %6s  %6s  %7s\n', 'Group', 'nChoi', 'Match', 'Mismatch', 'Rate(%)');
fprintf('%s\n', repmat('-', 1, 48));

% Build group name list from the same logic used in the data-loading loop.
grpNames = cell(1, nGroup);
for iGrp = 1 : nGroup
    if iGrp == 1, grpNames{iGrp} = 'younger';
    else,         grpNames{iGrp} = 'older';
    end
end

consistByPos = NaN(nGroup, nTrans);   % mean match rate per group per position

for iGrp = 1 : nGroup
    allC = [];   % all pickupConsistent values pooled across subjects/trials/positions
    posC = NaN(0, nTrans);

    for iSub = 1 : length(all_mouse_both_grp{iGrp})
        md = all_mouse_both_grp{iGrp}{iSub};
        for iRep = 1 : length(md)
            c = md(iRep).pickupConsistent;   % [1×nTrans] or empty
            if isempty(c), continue; end
            allC   = [allC, c(:)'];
            posC   = [posC; c(:)'];
        end
    end

    nTotal    = sum(~isnan(allC));
    nMatch    = sum(allC == 1, 'omitnan');
    nMismatch = sum(allC == 0, 'omitnan');
    rate      = 100 * nMatch / max(nTotal, 1);
    fprintf('%-12s  %6d  %6d  %8d  %6.1f\n', grpNames{iGrp}, nTotal, nMatch, nMismatch, rate);

    if ~isempty(posC)
        consistByPos(iGrp, :) = mean(posC, 1, 'omitnan');
    end
end
fprintf('%s\n\n', repmat('-', 1, 48));

% Figure: match rate per sequential position, one line per group
figure('Name', 'Pick-up consistency by position', 'Color', 'w', ...
       'Position', [100 100 340 240]), clf;
ax = gca; hold(ax, 'on');
grpClrs = lines(nGroup);
xPos    = 1 : nTrans;
for iGrp = 1 : nGroup
    plot(ax, xPos, consistByPos(iGrp,:) * 100, '-o', ...
        'Color', grpClrs(iGrp,:), 'LineWidth', 2, 'MarkerSize', 5, ...
        'DisplayName', grpNames{iGrp});
end
yline(ax, 100, '--k', 'LineWidth', 0.8, 'HandleVisibility', 'off');
ylim(ax, [0 105]);
xlabel(ax, 'Sequential position');
ylabel(ax, 'Match rate (%)');
title(ax, 'Pick-up consistency: trajectory vs response');
legend(ax, 'Location', 'southeast', 'Box', 'off');
set(ax, 'XTick', xPos, 'LineWidth', 1.5, 'FontSize', 12, ...
    'FontWeight', 'bold', 'FontName', 'Arial');

%% Plotting parameters
% figKey 0 → presentation (thick lines, axis labels visible)
% figKey 1 → Adobe Illustrator export (thin lines, no tick labels — add in AI)
figKey = 1;
if figKey == 0
    mainLineWid = 2;
    errLineWid  = 2;
    refLineWid  = 1;
    mrkSz       = 6;
    fntSz       = 15;
    axLineWid   = 2;
else
    mainLineWid = 1.5;
    errLineWid  = 1;
    refLineWid  = 0.5;
    mrkSz       = 4;
    fntSz       = 12;
    axLineWid   = 1.0;
end
figPos = [100 100 280 180];   % pixels: left bottom width height

% Inline style helper — applied after plotting in every figure
applyAxStyle = @(ax) set(ax, ...
    'LineWidth', axLineWid, 'FontSize', fntSz, ...
    'FontWeight', 'bold', 'FontName', 'Arial', ...
    'TickDir', 'out', 'Box', 'off');

condNames = {'Object', 'Location', 'Full'};
grpColors = [248, 218, 172; ...     % YA
             184, 204, 225] ./ 255; % OA
grpLabels = {'YA', 'OA'};
dataVars  = {all_mouse_con_grp, all_mouse_loc_grp, all_mouse_both_grp};

%% Figs 1–6: mindChanged and mindChanges — one figure per metric × condition
% plotSpec = struct( ...
%     'field',  {'mindChanged',          'mindChanges'}, ...
%     'ylabel', {'P(mind change)',        'Mean # switches'}, ...
%     'ftitle', {'Hesitation (binary)',   'Hesitation (count)'} ...
% );
% 
% for iFig = 1 : 2
%     for iCond = 1 : 3
% 
%         figure('Name', [plotSpec(iFig).ftitle, ' – ', condNames{iCond}], ...
%                'Color', 'w', 'Position', figPos), clf;
%         ax = gca; hold(ax, 'on');
% 
%         allGrpMeans = NaN(nGroup, nTrans);
%         allGrpSEMs  = NaN(nGroup, nTrans);
% 
%         for iGrp = 1 : nGroup
%             mouse_grp = dataVars{iCond}{iGrp};
%             nSub_plt  = numel(mouse_grp);
%             subMeans  = NaN(nSub_plt, nTrans);
% 
%             for iSub = 1 : nSub_plt
%                 md = mouse_grp{iSub};
%                 if isempty(md), continue; end
%                 mc = vertcat(md.(plotSpec(iFig).field));
%                 if isempty(mc), continue; end
%                 subMeans(iSub, :) = mean(mc, 1, 'omitnan');
%             end
% 
%             nValid  = max(sum(~isnan(subMeans), 1), 1);   % [1×nTrans], per position
%             grpMean = mean(subMeans, 1, 'omitnan');
%             grpSEM  = std(subMeans, 0, 1, 'omitnan') ./ sqrt(nValid);
%             allGrpMeans(iGrp, :) = grpMean;
%             allGrpSEMs(iGrp, :)  = grpSEM;
% 
%             clr  = grpColors(iGrp, :);
%             xPos = 1 : nTrans;
%             errorbar(ax, xPos, grpMean, grpSEM, 'Color', 'k', 'LineStyle', 'none', ...
%                 'LineWidth', errLineWid, 'HandleVisibility', 'off');
%             plot(ax, xPos, grpMean, '-o', 'Color', clr, ...
%                 'LineWidth', mainLineWid, 'MarkerSize', mrkSz, ...
%                 'MarkerEdgeColor', 'k', 'MarkerFaceColor', clr, 'DisplayName', grpLabels{iGrp});
%         end
% 
%         xlim(ax, [0.5, nTrans + 0.5]);
%         yUp  = allGrpMeans + allGrpSEMs;
%         yLow = allGrpMeans - allGrpSEMs;
%         yUp  = yUp(~isnan(yUp));
%         yLow = yLow(~isnan(yLow));
%         if ~isempty(yUp)
%             yPad = max(0.1 * (max(yUp) - min(yLow)), 0.02);
%             ylim(ax, [max(0, min(yLow) - yPad), max(yUp) + yPad]);
%         end
%         if iFig == 2
%             if iCond == 1 || iCond == 2
%                 ylim([0,  4]);
%             elseif iCond == 3
%                 ylim([0,  8]);
%             end
%         end
% 
%         applyAxStyle(ax);
%         xticks(ax, 1 : nTrans);
%         xlabel(ax, 'Choice position');
%         ylabel(ax, plotSpec(iFig).ylabel);
%         legend(ax, grpLabels, 'Location', 'best', 'Box', 'off', 'FontSize', fntSz - 2);
%         if figKey == 0
%             title(ax, [plotSpec(iFig).ftitle, ' – ', condNames{iCond}]);
%         end
%     end
% end

%% Figs 7–9: dwell time closest to chosen stimulus — one figure per condition
% chanceLvl = 1 / (nTrans + nDtr);   % = 1/6 ≈ 0.167
% 
% for iCond = 1 : 3
% 
%     figure('Name', ['Dwell time chosen – ', condNames{iCond}], ...
%            'Color', 'w', 'Position', figPos), clf;
%     ax = gca; hold(ax, 'on');
% 
%     %plot(ax, [0.5, nTrans+0.5], [chanceLvl, chanceLvl], '--', ...
%     %     'Color', [0.6 0.6 0.6], 'LineWidth', refLineWid, 'HandleVisibility', 'off');
% 
%     allGrpMeans_chosen = NaN(nGroup, nTrans);
%     allGrpSEMs_chosen  = NaN(nGroup, nTrans);
% 
%     for iGrp = 1 : nGroup
%         mouse_grp = dataVars{iCond}{iGrp};
%         nSub_plt  = numel(mouse_grp);
%         subChosen = NaN(nSub_plt, nTrans);
% 
%         for iSub = 1 : nSub_plt
%             md = mouse_grp{iSub};
%             if isempty(md), continue; end
%             mc = vertcat(md.timeInChosen);
%             if isempty(mc), continue; end
%             subChosen(iSub, :) = mean(mc, 1, 'omitnan');
%         end
% 
%         nValid     = max(sum(~isnan(subChosen), 1), 1);   % [1×nTrans], per position
%         meanChosen = mean(subChosen, 1, 'omitnan');
%         semChosen  = std(subChosen,  0, 1, 'omitnan') ./ sqrt(nValid);
%         allGrpMeans_chosen(iGrp, :) = meanChosen;
%         allGrpSEMs_chosen(iGrp, :)  = semChosen;
% 
%         clr  = grpColors(iGrp, :);
%         xPos = 1 : nTrans;
%         errorbar(ax, xPos, meanChosen, semChosen, 'Color', 'k', 'LineStyle', 'none', ...
%             'LineWidth', errLineWid, 'HandleVisibility', 'off');
%         plot(ax, xPos, meanChosen, '-o', 'Color', clr, ...
%             'LineWidth', mainLineWid, 'MarkerSize', mrkSz, ...
%             'MarkerEdgeColor', 'k', 'MarkerFaceColor', clr, 'DisplayName', grpLabels{iGrp});
%     end
% 
%     xlim(ax, [0.5, nTrans + 0.5]);
%     yUp  = allGrpMeans_chosen + allGrpSEMs_chosen;
%     yLow = allGrpMeans_chosen - allGrpSEMs_chosen;
%     yUp  = [yUp(~isnan(yUp));  chanceLvl];
%     yLow = [yLow(~isnan(yLow)); chanceLvl];
%     if ~isempty(yUp)
%         yPad = max(0.1 * (max(yUp) - min(yLow)), 0.02);
%         ylim(ax, [max(0, min(yLow) - yPad), min(1, max(yUp) + yPad)]);
%     end
%     ylim([0, 1]);
%     plot(xlim, [0.5, 0.5], 'k:', 'LineWidth', 0.8); hold on;
% 
%     applyAxStyle(ax);
%     xticks(ax, 1 : nTrans);
%     xlabel(ax, 'Choice position');
%     ylabel(ax, {'Dwell time', '(chosen fraction)'});
%     legend(ax, grpLabels, 'Location', 'best', 'Box', 'off', 'FontSize', fntSz - 2);
%     if figKey == 0
%         title(ax, ['Dwell time chosen – ', condNames{iCond}]);
%     end
% end


%% Figs 10–18: Interclick time, selection speed, dwell time — correct vs incorrect
% For each of the three new metrics × three conditions:
%   solid line  = correct selections
%   dashed line = incorrect selections
%   colours     = YA (warm) / OA (cool), matching earlier figures
%
% Per subject × position: mean across trials with isCorrect==1 (or ==0).
% Positions with no trials of a given correctness stay NaN (omitnan handles).

newMetrics = struct( ...
    'field',  {'interClickTime',   'selSpeed',              'dwellTime'}, ...
    'ylabel', {'Interclick time (s)', 'Selection speed (a.u./s)', 'Dwell time (s)'}, ...
    'ftitle', {'Interclick time',  'Selection speed',       'Dwell time (chosen)'} ...
);

for iMet = 1 : 3
    for iCond = 1 : 3

        figure('Name', [newMetrics(iMet).ftitle, ' – ', condNames{iCond}, ' (corr vs incorr)'], ...
               'Color', 'w', 'Position', figPos), clf;
        ax = gca; hold(ax, 'on');

        for iGrp = 1 : nGroup
            mouse_grp = dataVars{iCond}{iGrp};
            nSub_plt  = numel(mouse_grp);

            subCorr   = NaN(nSub_plt, nTrans);
            subIncorr = NaN(nSub_plt, nTrans);

            for iSub = 1 : nSub_plt
                md = mouse_grp{iSub};
                if isempty(md), continue; end
                mc_met = vertcat(md.(newMetrics(iMet).field));   % [nTrials×nTrans]
                mc_cor = vertcat(md.isCorrect);                  % [nTrials×nTrans]
                if isempty(mc_met) || isempty(mc_cor), continue; end

                for k = 1 : nTrans
                    cIdx = mc_cor(:, k) == 1;
                    iIdx = mc_cor(:, k) == 0;
                    if any(cIdx), subCorr(iSub,   k) = mean(mc_met(cIdx, k), 'omitnan'); end
                    if any(iIdx), subIncorr(iSub, k) = mean(mc_met(iIdx, k), 'omitnan'); end
                end
            end

            nValidCorr   = max(sum(~isnan(subCorr),   1), 1);   % [1×nTrans]
            nValidIncorr = max(sum(~isnan(subIncorr), 1), 1);
            clr    = grpColors(iGrp, :);
            xPos   = 1 : nTrans;

            grpMeanCorr   = mean(subCorr,   1, 'omitnan');
            grpSEMCorr    = std(subCorr,    0, 1, 'omitnan') ./ sqrt(nValidCorr);
            grpMeanIncorr = mean(subIncorr, 1, 'omitnan');
            grpSEMIncorr  = std(subIncorr,  0, 1, 'omitnan') ./ sqrt(nValidIncorr);

            errorbar(ax, xPos, grpMeanCorr, grpSEMCorr, ...
                'Color', 'k', 'LineStyle', '-', 'LineWidth', errLineWid, ... 
                'HandleVisibility', 'off');
            h1 = plot(ax, xPos, grpMeanCorr, 'Color', clr, 'Marker', 'o', 'MarkerSize', 7, ...
                'MarkerEdgeColor', 'k', 'MarkerFaceColor', clr, ...
                'LineStyle', '-', 'LineWidth', 2, ...
                'DisplayName', [grpLabels{iGrp}, ' correct']);

            errorbar(ax, xPos, grpMeanIncorr, grpSEMIncorr, ...
                'Color', 'k', 'LineStyle', ':', 'LineWidth', errLineWid, ...
                'HandleVisibility', 'off');
            h2 = plot(ax, xPos, grpMeanIncorr, 'Color', clr, 'Marker', 'o', 'MarkerSize', 7, ...
                'MarkerEdgeColor', 'k', 'MarkerFaceColor', 'none', ... 
                'LineStyle', ':', 'LineWidth', 2, ...
                'DisplayName', [grpLabels{iGrp}, ' incorrect']);
        end

        xlim(ax, [0.5, nTrans + 0.5]);
        applyAxStyle(ax);
        xticks(ax, 1 : nTrans);
        xlabel(ax, 'Choice position');
        ylabel(ax, newMetrics(iMet).ylabel);
        if figKey == 0
            title(ax, [newMetrics(iMet).ftitle, ' – ', condNames{iCond}]);
        end
        if iCond ~= 3 % full retrieval
            if iMet == 1
                ylim([0, 5]);
            elseif iMet == 2
                ylim([0, 1.2]);
            elseif iMet == 3
                ylim([0, 2.5]);
            end
        elseif iCond == 3 % full retrieval
            if iMet == 1
                ylim([0, 6]);
            elseif iMet == 2
                ylim([0, 1.2]);
            elseif iMet == 3
                ylim([0, 0.5]);
            end
        end
        
    end
end

%% Figs 19–27: New measures collapsed across transitions — correct vs incorrect
% One figure per metric × condition.  x-axis: correct | incorrect (2 positions).
% YA and OA shown side-by-side within each x-position.
% Each dot = one subject's mean across all trials (using the per-trial _corr /
% _incorr scalars already stored in mouse_data).  Error bar = SEM across subjects.

% Within each age group (x-position), correct and incorrect are placed
% side-by-side: correct at xBase-0.15, incorrect at xBase+0.15.
% A paired t-test (within-subject) is run per group; significance bracket
% is drawn above each pair after all data is plotted.
ciOffset = [-0.15, 0.15];   % [correct, incorrect] x-offset within each group

for iMet = 1 : 3
    corrField   = [newMetrics(iMet).field, '_corr'];
    incorrField = [newMetrics(iMet).field, '_incorr'];

    for iCond = 1 : 3

        figure('Name', [newMetrics(iMet).ftitle, ' – ', condNames{iCond}, ' collapsed'], ...
               'Color', 'w', 'Position', figPos), clf;
        ax = gca; hold(ax, 'on');


        % Storage for bracket drawing (filled after group loop)
        xCArr      = NaN(1, nGroup);
        xIArr      = NaN(1, nGroup);
        pArr       = NaN(1, nGroup);
        yTopArr    = NaN(1, nGroup);
        allSubCorr = cell(nGroup, 1);   % kept for between-group tests
        allSubIncorr = cell(nGroup, 1);

        for iGrp = 1 : nGroup
            mouse_grp = dataVars{iCond}{iGrp};
            nSub_plt  = numel(mouse_grp);
            clr       = grpColors(iGrp, :);

            subCorr   = NaN(nSub_plt, 1);
            subIncorr = NaN(nSub_plt, 1);

            for iSub = 1 : nSub_plt
                md = mouse_grp{iSub};
                if isempty(md), continue; end
                vc = [md.(corrField)];
                vi = [md.(incorrField)];
                subCorr(iSub)   = mean(vc(~isnan(vc)));
                subIncorr(iSub) = mean(vi(~isnan(vi)));
            end

            allSubCorr{iGrp}   = subCorr;
            allSubIncorr{iGrp} = subIncorr;

            xBase = iGrp;
            xC    = xBase + ciOffset(1);
            xI    = xBase + ciOffset(2);

            nC  = sum(~isnan(subCorr));
            nI  = sum(~isnan(subIncorr));
            mC  = mean(subCorr,   'omitnan');
            seC = std(subCorr,  0, 1, 'omitnan') / sqrt(max(nC, 1));
            mI  = mean(subIncorr, 'omitnan');
            seI = std(subIncorr, 0, 1, 'omitnan') / sqrt(max(nI, 1));

            % Individual subject dots
            scatter(ax, xC * ones(nSub_plt, 1), subCorr, ...
                25, clr, 'filled', 'MarkerFaceAlpha', 0.4, 'HandleVisibility', 'off');
            scatter(ax, xI * ones(nSub_plt, 1), subIncorr, ...
                25, clr, 'filled', 'MarkerFaceAlpha', 0.4, 'HandleVisibility', 'off');

            % Group mean ± SEM: filled = correct, open = incorrect
            errorbar(ax, xC, mC, seC, 'Color', 'k', 'LineStyle', 'none', ...
                'LineWidth', errLineWid, 'HandleVisibility', 'off');
            hC = plot(ax, xC, mC, 'o', 'Color', clr, ...
                'MarkerSize', mrkSz + 2, 'MarkerEdgeColor', 'k', 'MarkerFaceColor', clr, ...
                'LineWidth', mainLineWid, 'HandleVisibility', 'off');

            errorbar(ax, xI, mI, seI, 'Color', 'k', 'LineStyle', 'none', ...
                'LineWidth', errLineWid, 'HandleVisibility', 'off');
            hI = plot(ax, xI, mI, 'o', 'Color', clr, ...
                'MarkerSize', mrkSz + 2, 'MarkerEdgeColor', 'k', 'MarkerFaceColor', 'w', ...
                'LineWidth', mainLineWid, 'HandleVisibility', 'off');

            % Line connecting paired means
            plot(ax, [xC, xI], [mC, mI], '-', 'Color', clr, ...
                'LineWidth', mainLineWid * 0.7, 'HandleVisibility', 'off');

            % Within-group paired t-test (correct vs incorrect)
            validBoth = ~isnan(subCorr) & ~isnan(subIncorr);
            if sum(validBoth) > 1
                [~, p] = ttest(subCorr(validBoth), subIncorr(validBoth));
            else
                p = NaN;
            end

            % Track top of data for bracket placement
            allVals = [subCorr(~isnan(subCorr)); subIncorr(~isnan(subIncorr))];
            yTopArr(iGrp) = max([mC + seC, mI + seI, allVals(:)']);
            xCArr(iGrp)   = xC;
            xIArr(iGrp)   = xI;
            pArr(iGrp)    = p;
        end

        % Fixed y-axis ceiling per metric (user-specified); lower bound = 0.
        % Bracket positions are anchored to this ceiling so the trailing ylim
        % override is no longer needed.
        yFixedCeil = [5, 2, 4];   % iMet = 1,2,3
        yBot       = 0;
        yTop_data  = yFixedCeil(iMet);
        yRange     = yTop_data - yBot;
        tickLen    = 0.030 * yRange;
        textOff    = 0.025 * yRange;

        % Set ylim to fixed ceiling + bracket headroom BEFORE drawing annotations
        yFinal = yTop_data + 0.60 * yRange;
        ylim(ax, [yBot, yFinal]);

        % Level 1: within-group brackets (correct vs incorrect), one per group
        bracketY_w = yTop_data + 0.12 * yRange;
        for iGrp = 1 : nGroup
            if isnan(pArr(iGrp)), continue; end
            sigStr = pToSigStr(pArr(iGrp));
            xC = xCArr(iGrp);  xI = xIArr(iGrp);
            drawBracket(ax, xC, xI, bracketY_w, tickLen, textOff, sigStr, 'k', 0.8, fntSz);
        end

        % Level 2: between-group (age) comparisons — one per correctness condition.
        %   Correct bracket:   YA-correct   (xCArr(1)) → OA-correct   (xCArr(2))
        %   Incorrect bracket: YA-incorrect (xIArr(1)) → OA-incorrect (xIArr(2))
        %   Each uses an independent-samples t-test on per-subject means.
        %   The two x-ranges overlap, so brackets are drawn at staggered heights.
        vCYA = allSubCorr{1};   vCYA = vCYA(~isnan(vCYA));
        vCOA = allSubCorr{2};   vCOA = vCOA(~isnan(vCOA));
        vIYA = allSubIncorr{1}; vIYA = vIYA(~isnan(vIYA));
        vIOA = allSubIncorr{2}; vIOA = vIOA(~isnan(vIOA));

        pAgeCorr   = NaN;
        pAgeIncorr = NaN;
        if numel(vCYA) > 1 && numel(vCOA) > 1
            [~, pAgeCorr]   = ttest2(vCYA, vCOA);
        end
        if numel(vIYA) > 1 && numel(vIOA) > 1
            [~, pAgeIncorr] = ttest2(vIYA, vIOA);
        end

        % Between-group (age) statistical brackets omitted from figure.

        xlim(ax, [0.5, nGroup + 0.5]);
        xticks(ax, 1 : nGroup);
        xticklabels(ax, grpLabels);
        applyAxStyle(ax);
        ylabel(ax, newMetrics(iMet).ylabel);
        if figKey == 0
            title(ax, [newMetrics(iMet).ftitle, ' – ', condNames{iCond}, ' (collapsed)']);
        end
    end
end


%% Figs 28–36: Sequential correctness — metric by (prev response, curr response)
% For selections k = 2 : nTrans, each (trial, position) is classified by the
% pair (isCorrect(k-1), isCorrect(k)) into four categories:
%   CC  prev correct   → current correct
%   IC  prev incorrect → current correct
%   CI  prev correct   → current incorrect
%   II  prev incorrect → current incorrect
%
% x-axis: the four categories (CC, IC, CI, II).
%   CC and IC are shaded green  (current correct);
%   CI and II are shaded red    (current incorrect).
% YA / OA are shown side-by-side at each x-position.
% Significance brackets (paired t-test across subjects):
%   CC vs IC  — does previous correctness matter when currently correct?
%   CI vs II  — does previous correctness matter when currently incorrect?
% YA bracket is drawn lower; OA bracket is drawn higher so they don't overlap.

% seqLabels = {'Prev-C→Curr-C', 'Prev-I→Curr-C', 'Prev-C→Curr-I', 'Prev-I→Curr-I'};
% nSeq      = 4;
% grpOff_seq = [-0.12, 0.12];   % YA / OA offset within each x-position
% % Heights for significance brackets (as multiples of yRange above yDataMax)
% bracketLvl = [0.10, 0.22];    % YA = lower, OA = higher
% 
% for iMet = 1 : 3
%     metField = newMetrics(iMet).field;
% 
%     for iCond = 1 : 3
% 
%         figure('Name', [newMetrics(iMet).ftitle, ' – ', condNames{iCond}, ' sequential'], ...
%                'Color', 'w', 'Position', figPos), clf;
%         ax = gca; hold(ax, 'on');
% 
%         % Background shading is drawn after ylim is set (see below) so that
%         % patch y-coordinates don't distort the auto-scaled axis.
% 
%         hGrp    = gobjects(nGroup, 1);   % one handle per group for legend
%         xPosAll = NaN(nGroup, nSeq);     % x positions for bracket drawing
%         subAll  = cell(nGroup, 1);       % [nSub × 4] per-subject means
%         yTopAll = NaN(nGroup, nSeq);     % y maxima for bracket placement
% 
%         for iGrp = 1 : nGroup
%             mouse_grp = dataVars{iCond}{iGrp};
%             nSub_plt  = numel(mouse_grp);
%             clr       = grpColors(iGrp, :);
% 
%             subMat = NaN(nSub_plt, nSeq);   % cols: CC, IC, CI, II
% 
%             for iSub = 1 : nSub_plt
%                 md = mouse_grp{iSub};
%                 if isempty(md), continue; end
%                 mc_met = vertcat(md.(metField));   % [nTrials × nTrans]
%                 mc_cor = vertcat(md.isCorrect);    % [nTrials × nTrans]
%                 if isempty(mc_met) || isempty(mc_cor), continue; end
% 
%                 vCC = []; vIC = []; vCI = []; vII = [];
%                 for k = 2 : nTrans
%                     prev = mc_cor(:, k-1);   % NaN if not reached
%                     curr = mc_cor(:, k);
%                     met  = mc_met(:, k);
%                     % NaN==1 is false, so unresponded positions are excluded
%                     vCC = [vCC;  met(prev==1 & curr==1)]; %#ok<AGROW>
%                     vIC = [vIC;  met(prev==0 & curr==1)]; %#ok<AGROW>
%                     vCI = [vCI;  met(prev==1 & curr==0)]; %#ok<AGROW>
%                     vII = [vII;  met(prev==0 & curr==0)]; %#ok<AGROW>
%                 end
%                 subMat(iSub, 1) = mean(vCC, 'omitnan');
%                 subMat(iSub, 2) = mean(vIC, 'omitnan');
%                 subMat(iSub, 3) = mean(vCI, 'omitnan');
%                 subMat(iSub, 4) = mean(vII, 'omitnan');
%             end
% 
%             subAll{iGrp} = subMat;
% 
%             for iSeq = 1 : nSeq
%                 xPos = iSeq + grpOff_seq(iGrp);
%                 xPosAll(iGrp, iSeq) = xPos;
%                 vals = subMat(:, iSeq);
%                 nV   = sum(~isnan(vals));
%                 mV   = mean(vals, 'omitnan');
%                 seV  = std(vals, 0, 1, 'omitnan') / sqrt(max(nV, 1));
% 
%                 % Individual subject dots
%                 scatter(ax, xPos * ones(nSub_plt, 1), vals, 16, clr, ...
%                     'filled', 'MarkerFaceAlpha', 0.35, 'HandleVisibility', 'off');
% 
%                 % Group mean ± SEM
%                 errorbar(ax, xPos, mV, seV, 'Color', 'k', 'LineStyle', 'none', ...
%                     'LineWidth', errLineWid, 'HandleVisibility', 'off');
%                 h = plot(ax, xPos, mV, 'o', 'Color', clr, ...
%                     'MarkerSize', mrkSz + 1, 'MarkerEdgeColor', 'k', ...
%                     'MarkerFaceColor', clr, 'LineWidth', mainLineWid, ...
%                     'HandleVisibility', 'off');
% 
%                 yTopAll(iGrp, iSeq) = max([mV + seV; vals(~isnan(vals))]);
% 
%                 if iSeq == 1   % first sequence condition: register legend handle
%                     hGrp(iGrp) = h;
%                     set(hGrp(iGrp), 'HandleVisibility', 'on', ...
%                         'DisplayName', grpLabels{iGrp});
%                 end
%             end
% 
%             % Connect means within current-correct pair (CC–IC) and
%             % current-incorrect pair (CI–II) to show within-category trend
%             mV_all = mean(subMat, 1, 'omitnan');
%             plot(ax, xPosAll(iGrp, 1:2), mV_all(1:2), '-', 'Color', clr, ...
%                 'LineWidth', mainLineWid * 0.7, 'HandleVisibility', 'off');
%             plot(ax, xPosAll(iGrp, 3:4), mV_all(3:4), '-', 'Color', clr, ...
%                 'LineWidth', mainLineWid * 0.7, 'HandleVisibility', 'off');
%         end
% 
%         % --- Significance brackets (after all data is plotted) ---
%         yDataMax = max(yTopAll(:));
%         yCurr    = ylim(ax);
%         yRange   = yDataMax - yCurr(1);
%         tickLen  = 0.025 * yRange;
%         textOff  = 0.020 * yRange;
% 
%         % Two comparisons: CC vs IC (cols 1,2) and CI vs II (cols 3,4)
%         sigPairs = [1, 2; 3, 4];
% 
%         for iGrp = 1 : nGroup
%             bracY = yDataMax + bracketLvl(iGrp) * yRange;
% 
%             for iPair = 1 : 2
%                 c1 = sigPairs(iPair, 1);
%                 c2 = sigPairs(iPair, 2);
%                 x1 = xPosAll(iGrp, c1);
%                 x2 = xPosAll(iGrp, c2);
%                 sm = subAll{iGrp};
% 
%                 validBoth = ~isnan(sm(:, c1)) & ~isnan(sm(:, c2));
%                 if sum(validBoth) > 1
%                     [~, p] = ttest(sm(validBoth, c1), sm(validBoth, c2));
%                 else
%                     continue;
%                 end
% 
%                 if     p < 0.001, sigStr = '***';
%                 elseif p < 0.01,  sigStr = '**';
%                 elseif p < 0.05,  sigStr = '*';
%                 else,             sigStr = 'ns';
%                 end
% 
%                 clr = grpColors(iGrp, :);
%                 plot(ax, [x1, x2], [bracY, bracY], '-', 'Color', clr, ...
%                     'LineWidth', 1.0, 'HandleVisibility', 'off');
%                 plot(ax, [x1, x1], [bracY - tickLen, bracY], '-', 'Color', clr, ...
%                     'LineWidth', 1.0, 'HandleVisibility', 'off');
%                 plot(ax, [x2, x2], [bracY - tickLen, bracY], '-', 'Color', clr, ...
%                     'LineWidth', 1.0, 'HandleVisibility', 'off');
%                 text(ax, (x1 + x2) / 2, bracY + textOff, sigStr, ...
%                     'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', ...
%                     'FontSize', fntSz, 'FontName', 'Arial', 'Color', clr);
%             end
%         end
% 
%         % Expand y-axis to clear all brackets and labels
%         yTop = yDataMax + (bracketLvl(end) + 0.15) * yRange;
%         ylim(ax, [yCurr(1), yTop]);
% 
%         % Now draw background shading with correct y limits, then push to back
%         yL = ylim(ax);
%         p1 = patch(ax, [0.5 2.5 2.5 0.5], [yL(1) yL(1) yL(2) yL(2)], [0.93 0.98 0.93], ...
%             'EdgeColor', 'none', 'FaceAlpha', 0.6, 'HandleVisibility', 'off');
%         p2 = patch(ax, [2.5 4.5 4.5 2.5], [yL(1) yL(1) yL(2) yL(2)], [0.98 0.93 0.93], ...
%             'EdgeColor', 'none', 'FaceAlpha', 0.6, 'HandleVisibility', 'off');
%         uistack(p1, 'bottom');
%         uistack(p2, 'bottom');
% 
%         % Vertical divider between current-correct and current-incorrect halves
%         plot(ax, [2.5 2.5], ylim(ax), '--', 'Color', [0.6 0.6 0.6], ...
%             'LineWidth', 0.8, 'HandleVisibility', 'off');
% 
%         % x-axis labels: two-line tick labels via text() annotations
%         xlim(ax, [0.5, nSeq + 0.5]);
%         xticks(ax, 1 : nSeq);
%         xticklabels(ax, seqLabels);
%         ax.XTickLabelRotation = 20;
% 
%         applyAxStyle(ax);
%         ylabel(ax, newMetrics(iMet).ylabel);
%         legend(ax, hGrp, grpLabels, 'Location', 'best', 'Box', 'off', ...
%             'FontSize', fntSz - 2);
%         if figKey == 0
%             title(ax, [newMetrics(iMet).ftitle, ' – ', condNames{iCond}, ' (sequential)']);
%         end
%     end
% end


%% Example participant: mouse trajectory and distance to stimuli over time
% Fig 5 — x-y trajectory colored by time (cool colormap); stimuli/slots as squares
% Fig 6 — distance to each stimulus/slot over time; one colored line per stimulus
%
% Example trial selection: first YA subject that has valid trajectory + distance
% data for all three retrieval conditions; first valid trial per condition.

imgSize_plot = 0.15;   % image width in PsychoJS normalized units
sqHW         = imgSize_plot / 2;   % half-width for drawn squares

% ---- Select example participant and trial (edit these three lines) ----
% exGrp:  1 = younger adults, 2 = older adults
% exSub:  subject index within the group (1-based)
% exTrial: trial index (1-based, same for all three conditions);
%          set to 0 to auto-pick the first trial valid in ALL conditions
exGrp   = 1;
exSub   = 1;
exTrial = 1;

md_c = all_mouse_con_grp{exGrp}{exSub};
md_l = all_mouse_loc_grp{exGrp}{exSub};
md_b = all_mouse_both_grp{exGrp}{exSub};

if exTrial == 0
    % Find first trial index valid in ALL three conditions simultaneously
    nT_min = min([numel(md_c), numel(md_l), numel(md_b)]);
    validC   = ~cellfun(@isempty, {md_c.trajectory}) & ~cellfun(@isempty, {md_c.distToStim}) & ~cellfun(@isempty, {md_c.clickTimes});
    validL   = ~cellfun(@isempty, {md_l.trajectory}) & ~cellfun(@isempty, {md_l.distToStim}) & ~cellfun(@isempty, {md_l.clickTimes});
    validB   = ~cellfun(@isempty, {md_b.trajectory}) & ~cellfun(@isempty, {md_b.distToSlot}) & ~cellfun(@isempty, {md_b.clickTimes});
    validAll = validC(1:nT_min) & validL(1:nT_min) & validB(1:nT_min);
    exTrial  = find(validAll, 1);
end

% Check validity of chosen trial in each condition
conOk  = ~isempty(exTrial) && exTrial <= numel(md_c) && ...
          ~isempty(md_c(exTrial).trajectory) && ~isempty(md_c(exTrial).distToStim)  && ~isempty(md_c(exTrial).clickTimes);
locOk  = ~isempty(exTrial) && exTrial <= numel(md_l) && ...
          ~isempty(md_l(exTrial).trajectory) && ~isempty(md_l(exTrial).distToStim)  && ~isempty(md_l(exTrial).clickTimes);
bothOk = ~isempty(exTrial) && exTrial <= numel(md_b) && ...
          ~isempty(md_b(exTrial).trajectory) && ~isempty(md_b(exTrial).distToSlot)  && ~isempty(md_b(exTrial).clickTimes);

if isempty(exTrial) || ~conOk || ~locOk || ~bothOk
    warning('Trial %d not valid for all conditions (grp=%d, sub=%d); skipping trajectory figures.', ...
        double(~isempty(exTrial)) * exTrial, exGrp, exSub);
else
    exData = { md_c(exTrial), md_l(exTrial), md_b(exTrial) };
    % For 'both', analysis uses distance to target slots; con/loc use distToStim
    distFieldEx = {'distToStim', 'distToStim', 'distToSlot'};

    nStim    = nTrans + nDtr;          % 6
    stimCmap = lines(nStim);           % 6 distinct colors, one per stimulus/slot

    %% Figs 16–18: Mouse trajectory colored by time — one figure per condition
    for iCond = 1 : 3

        md   = exData{iCond};
        traj = md.trajectory;

        if iCond == 3
            % Both report: side-by-side panels for content (inner images) and
            % location (outer slots) dimensions.
            figure('Name', ['Trajectory – ', condNames{iCond}], ...
                   'Color', 'w', 'Position', [100 100 720 320]), clf;
            axList = {subplot(1,2,1), subplot(1,2,2)};
            panelTitles = {'Content dimension (images)', 'Location dimension (slots)'};
        else
            figure('Name', ['Trajectory – ', condNames{iCond}], ...
                   'Color', 'w', 'Position', [100 100 360 320]), clf;
            axList = {gca};
            panelTitles = {['Trajectory – ', condNames{iCond}]};
        end

        if isempty(traj)
            for iAx = 1:length(axList), applyAxStyle(axList{iAx}); end
            continue;
        end

        xT    = traj(:, 1);
        yT    = traj(:, 2);
        tT    = traj(:, 3);
        tNorm = (tT - tT(1)) / max(tT(end) - tT(1), eps);
        trueOrd = md.trueOrder;
        clickT  = md.clickTimes;

        % For 'both': split trajectory samples into pick-up vs drag phase using
        % the same criterion as in the analysis — a sample is in the pick-up
        % phase (content panel) when the cursor is closer to any inner-circle
        % image than to any outer slot; drag phase (location panel) otherwise.
        if iCond == 3 && ~isempty(md.distToStim) && ~isempty(md.distToSlot)
            minStim     = min(md.distToStim, [], 2);   % [nT × 1]
            minSlot     = min(md.distToSlot, [], 2);
            phaseMask   = {minStim < minSlot, minStim >= minSlot};  % {pickup, drag}
        else
            phaseMask   = {true(size(xT)), true(size(xT))};
        end

        nPanels = length(axList);
        for iPanel = 1 : nPanels
            ax = axList{iPanel};
            hold(ax, 'on');

            % For 'both': each panel shows only the trajectory samples for that phase
            if iCond == 3
                mask  = phaseMask{iPanel};
                xPlot = xT(mask);  yPlot = yT(mask);  cPlot = tNorm(mask);
            else
                xPlot = xT;  yPlot = yT;  cPlot = tNorm;
            end

            scatter(ax, xPlot, yPlot, 10, cPlot, 'filled', 'MarkerFaceAlpha', 0.8);
            colormap(ax, 'cool');
            cb = colorbar(ax);
            cb.Label.String = 'Time (norm.)';
            caxis(ax, [0, 1]);

            % Choose which stimulus positions to draw for this panel
            if iCond == 3 && iPanel == 1
                % Content dimension: inner-circle image positions (solid squares)
                pX = md.stimX;  pY = md.stimY;
                rectStyle = '-';   rectLW = 2;
            elseif iCond == 3 && iPanel == 2
                % Location dimension: outer-circle slot positions (dashed squares)
                pX = md.slotX;  pY = md.slotY;
                rectStyle = '--';  rectLW = 1.5;
            else
                pX = md.stimX;  pY = md.stimY;
                rectStyle = '-';   rectLW = 2;
            end

            if ~isempty(pX)
                for iS = 1 : length(pX)
                    rectangle(ax, 'Position', ...
                        [pX(iS)-sqHW, pY(iS)-sqHW, 2*sqHW, 2*sqHW], ...
                        'EdgeColor', stimCmap(iS,:), 'LineWidth', rectLW, ...
                        'FaceColor', 'none', 'LineStyle', rectStyle);
                    if ~isempty(trueOrd) && iS <= length(trueOrd)
                        tOrdVal = trueOrd(iS);
                        lbl = 'dtr';
                        if tOrdVal ~= nTrans + nDtr, lbl = num2str(tOrdVal); end
                    else
                        lbl = num2str(iS);
                    end
                    text(ax, pX(iS), pY(iS) - sqHW - 0.02, lbl, ...
                        'Color', stimCmap(iS,:), 'FontSize', fntSz - 1, ...
                        'FontWeight', 'bold', 'FontName', 'Arial', ...
                        'HorizontalAlignment', 'center', 'VerticalAlignment', 'top', ...
                        'BackgroundColor', 'w', 'Margin', 1);
                end
            end

            % Drop-time markers: only shown in the location panel for 'both'
            % (drops occur at the end of the drag phase), in both panels otherwise.
            if ~isempty(clickT) && ~(iCond == 3 && iPanel == 1)
                for iK = 1 : length(clickT)
                    [~, tidx] = min(abs(tT - clickT(iK)));
                    plot(ax, xT(tidx), yT(tidx), 'v', ...
                        'Color', 'k', 'MarkerSize', mrkSz + 2, 'MarkerFaceColor', 'k');
                end
            end

            axis(ax, 'equal');
            applyAxStyle(ax);
            xlabel(ax, 'x (norm.)');
            ylabel(ax, 'y (norm.)');
            title(ax, panelTitles{iPanel});
        end
    end

    %% Figs 19–21: Distance to each stimulus/slot over time — one figure per condition
    for iCond = 1 : 3

        md   = exData{iCond};
        traj = md.trajectory;

        if iCond == 3
            % Both report: side-by-side panels for content (distToStim) and
            % location (distToSlot) dimensions.
            figure('Name', ['Distance – ', condNames{iCond}], ...
                   'Color', 'w', 'Position', [100 100 1100 280]), clf;
            distMats   = {md.distToStim, md.distToSlot};
            panelTitles = {'Content dimension (distance to images)', ...
                           'Location dimension (distance to slots)'};
        else
            figure('Name', ['Distance – ', condNames{iCond}], ...
                   'Color', 'w', 'Position', [100 100 600 280]), clf;
            distMats    = {md.(distFieldEx{iCond})};
            panelTitles = {['Distance – ', condNames{iCond}]};
        end

        if isempty(traj)
            applyAxStyle(gca); continue;
        end

        tT     = traj(:, 3);
        tOff   = (tT - tT(1)) / max(tT(end) - tT(1), eps);
        tRange = max(tT(end) - tT(1), eps);
        trueOrd = md.trueOrder;
        clickT  = md.clickTimes;

        % For 'both' content panel: restrict distance curve to pick-up phase
        % samples only (same criterion as Trim 2 in analyzeChoiceHesitation).
        pickupPhaseMask = true(size(tT));
        if iCond == 3 && ~isempty(md.distToStim) && ~isempty(md.distToSlot)
            minStim = min(md.distToStim, [], 2);
            minSlot = min(md.distToSlot, [], 2);
            pickupPhaseMask = minStim < minSlot;
        end

        nPanels = length(distMats);
        for iPanel = 1 : nPanels
            if iCond == 3
                ax = subplot(1, 2, iPanel);
            else
                ax = gca;
            end
            hold(ax, 'on');

            distMat = distMats{iPanel};
            if isempty(distMat), applyAxStyle(ax); continue; end

            for iS = 1 : size(distMat, 2)
                if ~isempty(trueOrd) && iS <= length(trueOrd)
                    tOrdVal = trueOrd(iS);
                    if tOrdVal == nTrans + nDtr
                        legLabel = sprintf('S%d (dtr)', iS);
                    else
                        legLabel = sprintf('S%d (true=%d)', iS, tOrdVal);
                    end
                else
                    legLabel = ['S', num2str(iS)];
                end

                plot(ax, tOff, distMat(:, iS), '-', ...
                    'Color', stimCmap(iS,:), 'LineWidth', mainLineWid, ...
                    'DisplayName', legLabel);
            end

            yTop = max(distMat(:)) * 1.1;
            if ~isempty(clickT)
                if iCond == 3 && iPanel == 1
                    % Content panel: mark P1–P5 at the END of each pick-up
                    % phase — the last sample in each inter-click window where
                    % the cursor is still closer to an inner image than any slot.
                    pClr = [0.6 0.2 0.8];
                    for iK = 1 : length(clickT)
                        tWinStart = -inf;
                        if iK > 1, tWinStart = clickT(iK - 1); end
                        winMask      = tT > tWinStart & tT <= clickT(iK);
                        lastPickupIdx = find(winMask & pickupPhaseMask, 1, 'last');
                        if isempty(lastPickupIdx), continue; end
                        tPick = (tT(lastPickupIdx) - tT(1)) / tRange;
                        if tPick < 0 || tPick > 1, continue; end
                        plot(ax, [tPick, tPick], [0, yTop], '--', ...
                            'Color', pClr, 'LineWidth', refLineWid, ...
                            'HandleVisibility', 'off');
                        text(ax, tPick, yTop * 0.96, ['P', num2str(iK)], ...
                            'FontSize', fntSz - 2, 'FontName', 'Arial', ...
                            'HorizontalAlignment', 'center', 'Color', pClr);
                    end
                else
                    % Location panel and all non-both panels: drop time markers.
                    for iK = 1 : length(clickT)
                        tClick = (clickT(iK) - tT(1)) / tRange;
                        if tClick < 0 || tClick > 1, continue; end
                        plot(ax, [tClick, tClick], [0, yTop], '--k', ...
                            'LineWidth', refLineWid, 'HandleVisibility', 'off');
                        text(ax, tClick, yTop * 0.96, ['C', num2str(iK)], ...
                            'FontSize', fntSz - 2, 'FontName', 'Arial', ...
                            'HorizontalAlignment', 'center', 'Color', 'k');
                    end
                end
            end

            xlim(ax, [0, 1]);
            ylim(ax, [0, yTop]);
            applyAxStyle(ax);
            legend(ax, 'Location', 'northeast', 'Box', 'off', 'FontSize', fntSz - 2);
            xlabel(ax, 'Time (norm.)');
            ylabel(ax, 'Distance (norm. units)');
            title(ax, panelTitles{iPanel});
        end
    end
end


%% ------ Define the function ------
function mouse_data = extractMouseClosestToReport(seqMem_subj, xName, yName, tName, reportName)
% Extract the rows containing the mouse trajectory from the .csv

    x_col = seqMem_subj.(xName);
    y_col = seqMem_subj.(yName);
    t_col = seqMem_subj.(tName);
    report_col = seqMem_subj.(reportName);

    % rows with valid report
    valid_report_rows = find(~ismissing(report_col) & strlength(report_col) > 0);

    % rows with valid mouse trajectory (raw availability only)
    valid_mouse_rows = find(~ismissing(x_col) & ~ismissing(y_col) & ~ismissing(t_col) & ...
                            strlength(x_col) > 0 & strlength(y_col) > 0 & strlength(t_col) > 0);

    nTrial = length(valid_report_rows);

    % -------- PREALLOCATE (important) --------
    mouse_data = struct( ...
        'rowReport', cell(nTrial,1), ...
        'rowMouse', cell(nTrial,1), ...
        'reportTrue', cell(nTrial,1), ...
        'trajectory', cell(nTrial,1) ...
    );

    for iRep = 1 : nTrial

        reportRow  = valid_report_rows(iRep);
        report_str = report_col(reportRow);

        mouse_data(iRep).rowReport  = reportRow;
        mouse_data(iRep).reportTrue = report_str;

        % ---------- find closest mouse row ----------
        if isempty(valid_mouse_rows)
            mouse_data(iRep).rowMouse   = NaN;
            mouse_data(iRep).trajectory = [];
            continue;
        end

        [~, idxClosest] = min(abs(valid_mouse_rows - reportRow));
        mouseRow = valid_mouse_rows(idxClosest);

        mouse_data(iRep).rowMouse = mouseRow;

        % ---------- parse ----------
        x_arr = parseMouseArray(x_col(mouseRow));
        y_arr = parseMouseArray(y_col(mouseRow));
        t_arr = parseMouseArray(t_col(mouseRow));

        % ---------- validity check ----------
        if isempty(x_arr) || isempty(y_arr) || isempty(t_arr) || ...
           length(x_arr) ~= length(y_arr) || length(x_arr) ~= length(t_arr)

            % DO NOT SKIP — mark as missing
            mouse_data(iRep).trajectory = [];

        else
            mouse_data(iRep).trajectory = [x_arr(:), y_arr(:), t_arr(:)];
        end
    end
end

function mouse_data = addStimPos(mouse_data, reportType, seqMem_subj, epi_rows, imgDispX_global, imgDispY_global)
% Add on-screen stimulus positions to each trial entry in mouse_data.
%
% Strategy: for each trial's report row, find the last episodeStart row
% (identified by a non-empty trialNo column) that precedes it — that row
% holds the per-trial position columns saved by the JS at trial onset.
%
% New fields added to each struct element:
%   .stimX / .stimY  – [1×6] positions of the 6 clickable / draggable stimuli
%                       con : fixed horizontal image slots (imgDispX/Y)
%                       loc : outer-circle slot positions (locSeqXTrl/Y)
%                       both: inner-circle draggable image positions (bothConPosXTrl/Y)
%   .slotX / .slotY  – [1×6] outer-circle slot positions (both report only, else [])
%   .trialNo         – scalar  0-based trial index from the JS

for iRep = 1 : length(mouse_data)

    % Defaults
    mouse_data(iRep).stimX   = [];
    mouse_data(iRep).stimY   = [];
    mouse_data(iRep).slotX   = [];
    mouse_data(iRep).slotY   = [];
    mouse_data(iRep).trialNo = NaN;

    reportRow = mouse_data(iRep).rowReport;
    if isempty(reportRow), continue; end

    % --- Content report: positions are identical every trial ---
    if strcmp(reportType, 'con')
        mouse_data(iRep).stimX = imgDispX_global;
        mouse_data(iRep).stimY = imgDispY_global;
        % trialNo: find the episodeStart row that precedes this report
        idx = find(epi_rows <= reportRow, 1, 'last');
        if ~isempty(idx)
            tNoStr = seqMem_subj.('trialNo')(epi_rows(idx));
            if ~ismissing(tNoStr) && strlength(tNoStr) > 0
                mouse_data(iRep).trialNo = str2double(char(tNoStr));
            end
        end
        continue;
    end

    % --- Location and Both reports: per-trial positions from episodeStart row ---
    idx = find(epi_rows <= reportRow, 1, 'last');
    if isempty(idx), continue; end
    epiRow = epi_rows(idx);

    % Record trial number
    tNoStr = seqMem_subj.('trialNo')(epiRow);
    if ~ismissing(tNoStr) && strlength(tNoStr) > 0
        mouse_data(iRep).trialNo = str2double(char(tNoStr));
    end

    % Outer-circle slot positions (shared by both loc and both reports)
    locX = parseMouseArray(seqMem_subj.('locSeqXTrl')(epiRow));  % [1×6]
    locY = parseMouseArray(seqMem_subj.('locSeqYTrl')(epiRow));  % [1×6]
    if isempty(locX) || isempty(locY), continue; end

    switch reportType
        case 'loc'
            % Participant clicks the empty circle slots
            mouse_data(iRep).stimX = locX;
            mouse_data(iRep).stimY = locY;

        case 'both'
            % Draggable images start at inner-circle positions
            conX = parseMouseArray(seqMem_subj.('bothConPosXTrl')(epiRow)); % [1×6]
            conY = parseMouseArray(seqMem_subj.('bothConPosYTrl')(epiRow)); % [1×6]
            if isempty(conX) || isempty(conY), continue; end
            mouse_data(iRep).stimX = conX;   % inner-circle image origins
            mouse_data(iRep).stimY = conY;
            mouse_data(iRep).slotX = locX;   % outer-circle target slots
            mouse_data(iRep).slotY = locY;
    end
end
end


function mouse_data = computeDistToStim(mouse_data)
% Compute Euclidean distance from every trajectory sample to every stimulus.
%
% For each trial, uses:
%   .trajectory  – [nT × 3]  columns x, y, t
%   .stimX/Y     – [1 × 6]   stimulus positions
%   .slotX/Y     – [1 × 6]   slot positions (both report only)
%
% Adds:
%   .distToStim  – [nT × 6]  distance to each stimulus at every sample
%   .distToSlot  – [nT × 6]  distance to each slot   (both report only)
%
% Column order in distToStim / distToSlot matches the slot order A–F as
% stored in stimX/Y (i.e., the JS display order for that trial).

for iRep = 1 : length(mouse_data)

    mouse_data(iRep).distToStim = []; % Object retrieval 
    mouse_data(iRep).distToSlot = [];

    traj  = mouse_data(iRep).trajectory;
    stimX = mouse_data(iRep).stimX;
    stimY = mouse_data(iRep).stimY;

    if isempty(traj) || isempty(stimX) || isempty(stimY)
        continue;
    end

    tx = traj(:, 1);   % [nT × 1]  mouse x
    ty = traj(:, 2);   % [nT × 1]  mouse y
    sx = stimX(:)';    % [1 × nStim]  broadcast across rows
    sy = stimY(:)';

    % Euclidean distance: broadcasting gives [nT × nStim]
    mouse_data(iRep).distToStim = sqrt((tx - sx).^2 + (ty - sy).^2);

    % Slot distances for the both report
    slotX = mouse_data(iRep).slotX;
    slotY = mouse_data(iRep).slotY;
    if ~isempty(slotX) && ~isempty(slotY)
        sx2 = slotX(:)';
        sy2 = slotY(:)';
        mouse_data(iRep).distToSlot = sqrt((tx - sx2).^2 + (ty - sy2).^2);
    end
end
end


function mouse_data = analyzeChoiceHesitation(mouse_data, seqMem_subj, reportType, nTrans, nDtr)
% For each retrieval trial, segment the mouse trajectory by click time,
% find which stimulus/slot was closest (argmin distToStim or distToSlot)
% at each sample within that segment, and count how many times the
% closest-stimulus identity switches before the participant's click.
%
% Per-click fields [1×nTrans]:
%   .chosenSlots    — 1-based stimulus indices in click order
%                     The index of the stimulus
%                     e.g., the candidate objects:    [1, 6, 4, 2, 3, 5]
%                     the fully correct .chosenSlots: [1, 4, 5, 3, 6]
%   .clickTimes     — RT (s) for each of the nTrans choices
%   .isCorrect      — 1 if click matched true sequence order, 0 if not, NaN if unknown
%   .mindChanges    — # closest-stimulus identity switches per deliberation window
%   .mindChanged    — binary: any switch (0/1)
%   .timeInChosen   — time-weighted fraction of deliberation time closest to chosen stim
%   .timeInOther    — 1 - timeInChosen
%   .closestAtClick — closest stim index at moment of click
%
% Accuracy-conditioned scalars (collapsed across all transitions per trial):
%   .mindChanges_corr    — mean mindChanges over correct clicks
%   .mindChanges_incorr  — mean mindChanges over incorrect clicks
%   .timeInChosen_corr   — mean timeInChosen over correct clicks
%   .timeInChosen_incorr — mean timeInChosen over incorrect clicks

switch reportType
    case 'con'
        ordColName  = 'conReportOrd';
        rtsColName  = 'conRTs';
        trueColName = 'conReportTrue';
        distField   = 'distToStim';
    case 'loc'
        ordColName  = 'locReportOrd';
        rtsColName  = 'locRTs';
        trueColName = 'locReportTrue';
        distField   = 'distToStim';
    case 'both'
        ordColName  = 'bothReportOrd';
        rtsColName  = 'bothRTs';
        trueColName = 'bothReportTrue';
        distField   = 'distToSlot';
end

for iRep = 1 : length(mouse_data)

    % Default empty outputs
    mouse_data(iRep).nResp               = 0;
    mouse_data(iRep).chosenSlots         = [];
    mouse_data(iRep).clickTimes          = [];
    mouse_data(iRep).isCorrect           = [];
    mouse_data(iRep).mindChanges         = [];
    mouse_data(iRep).mindChanged         = [];
    mouse_data(iRep).timeInChosen        = [];
    mouse_data(iRep).timeInOther         = [];
    mouse_data(iRep).closestAtClick      = [];
    mouse_data(iRep).mindChanges_corr      = NaN;
    mouse_data(iRep).mindChanges_incorr   = NaN;
    mouse_data(iRep).timeInChosen_corr    = NaN;
    mouse_data(iRep).timeInChosen_incorr  = NaN;
    mouse_data(iRep).interClickTime       = [];
    mouse_data(iRep).selSpeed             = [];
    mouse_data(iRep).dwellTime            = [];
    mouse_data(iRep).interClickTime_corr  = NaN;
    mouse_data(iRep).interClickTime_incorr= NaN;
    mouse_data(iRep).selSpeed_corr        = NaN;
    mouse_data(iRep).selSpeed_incorr      = NaN;
    mouse_data(iRep).dwellTime_corr       = NaN;
    mouse_data(iRep).dwellTime_incorr     = NaN;
    mouse_data(iRep).trueOrder            = [];   % [1×(nTrans+nDtr)] true order per slot

    traj      = mouse_data(iRep).trajectory;
    distMat   = mouse_data(iRep).(distField);
    reportRow = mouse_data(iRep).rowReport;
    distStimMat = [];
    if strcmp(reportType, 'both')
        distStimMat = mouse_data(iRep).distToStim;
    end

    if isempty(traj) || isempty(distMat) || isempty(reportRow)
        continue;
    end

    ordStr = seqMem_subj.(ordColName)(reportRow);
    rtsStr = seqMem_subj.(rtsColName)(reportRow);

    allNums = parseMouseArray(ordStr);
    repRTs  = parseMouseArray(rtsStr);

    mat = [];   % [nSlot×2] for 'both': col1=content order, col2=location order
    if strcmp(reportType, 'both')
        % bothReportOrd is a flattened [6×2] JS array (12 numbers).
        % reshape(., 2, 6)' → [6×2]: col1=content order, col2=location order per slot.
        if length(allNums) ~= 2 * (nTrans + nDtr)
            continue;
        end
        mat    = reshape(allNums, 2, 6)';  % [6×2]
        repOrd = mat(:, 2)';               % [1×6]: location drop order per slot
    else
        repOrd = allNums;
    end

    if length(repOrd) ~= nTrans + nDtr || length(repRTs) ~= nTrans + nDtr
        continue;
    end

    % Sort slots by click order (1 = first chosen, 2 = second, …)
    [sortedOrd, slotIdx] = sort(repOrd);
    validMask = sortedOrd >= 1 & sortedOrd <= nTrans;
    nResp = sum(validMask);
    if nResp == 0
        continue;
    end

    chosenSlots = slotIdx(validMask);    % [1×nResp], 1-based stimulus indices
    clickTimes  = repRTs(chosenSlots);   % [1×nResp], cumulative RTs in click order

    % Non-monotonic click times indicate a data-recording anomaly; the
    % segment windows would overlap or invert, so skip the trial.
    if any(diff(clickTimes) <= 0)
        continue;
    end

    % Interclick time: duration of each deliberation window.
    % Position 1 = time from trial start (t=0) to first selection;
    % position k = clickTimes(k) – clickTimes(k-1) for k > 1.
    interClickTime    = NaN(1, nResp);
    interClickTime(1) = clickTimes(1);
    if nResp > 1
        interClickTime(2:end) = diff(clickTimes);
    end

    % Determine per-click correctness.
    % con/loc: click k correct if trueNums(chosen slot) == k
    %          (distractor has trueNums = nTrans+1, never equals k=1..nTrans)
    % both: requires BOTH content AND location correct simultaneously:
    %          content: mat(s,1) == trueNums(s)  [reported content order matches truth]
    %          location: trueNums(s) == k         [true order matches click position k]
    trueStr  = seqMem_subj.(trueColName)(reportRow);
    trueNums = parseMouseArray(trueStr);
    if length(trueNums) == nTrans + nDtr
        mouse_data(iRep).trueOrder = trueNums(:)';
    end

    isCorrect = NaN(1, nResp);
    if length(trueNums) == nTrans + nDtr
        switch reportType
            case {'con', 'loc'}
                for k = 1 : nResp
                    s = chosenSlots(k);
                    isCorrect(k) = double(trueNums(s) == k);
                end
            case 'both'
                for k = 1 : nResp
                    s = chosenSlots(k);
                    if s >= 1 && s <= size(mat, 1)
                        conCorrect = (mat(s, 1) == trueNums(s));
                        locCorrect = (trueNums(s) == k);
                        isCorrect(k) = double(conCorrect && locCorrect);
                    end
                end
        end
    end

    txAll = traj(:, 1);
    tyAll = traj(:, 2);
    tAxis = traj(:, 3);

    mindChanges    = NaN(1, nResp);
    mindChanged    = NaN(1, nResp);
    timeInChosen   = NaN(1, nResp);
    timeInOther    = NaN(1, nResp);
    closestAtClick = NaN(1, nResp);
    selSpeed       = NaN(1, nResp);
    dwellTime      = NaN(1, nResp);
    pickupTime        = NaN(1, nResp);   % 'both' only: duration of pick-up phase per choice
    pickupConsistent  = NaN(1, nResp);   % 'both' only: 1=trajectory matches response, 0=mismatch

    for iChoice = 1 : nResp
        t_end   = clickTimes(iChoice);
        t_start = -inf;                         % first window: include all pre-click samples
        if iChoice > 1
            t_start = clickTimes(iChoice - 1);  % strict > excludes the previous click itself
        end

        segMask = tAxis > t_start & tAxis <= t_end;
        if sum(segMask) < 2
            continue;
        end

        tSeg    = tAxis(segMask);                 % [nSamp × 1] sample timestamps
        distSeg = distMat(segMask, :);            % [nSamp × nStim]
        [~, closestIdx] = min(distSeg, [], 2);    % [nSamp × 1]

        % Cursor path over the full interclick window.
        % For 'con'/'loc' selSpeed is computed here (over full window).
        % For 'both' selSpeed is deferred to after the drag-phase trim below.
        xSeg_full = txAll(segMask);
        ySeg_full = tyAll(segMask);
        if ~strcmp(reportType, 'both')
            pathLen = sum(sqrt(diff(xSeg_full).^2 + diff(ySeg_full).^2));
            if interClickTime(iChoice) > 0
                selSpeed(iChoice) = pathLen / interClickTime(iChoice);
            end
        end

        % Trim 1: remove the leading period where the cursor lingers on the
        % previously chosen stimulus (inertia, not deliberation about the next).
        trimStart = 1;
        if iChoice > 1
            prevChosen = chosenSlots(iChoice - 1);
            leftPrev   = find(closestIdx ~= prevChosen, 1, 'first');
            if isempty(leftPrev)
                continue;
            end
            trimStart = leftPrev;
        end
        closestIdx = closestIdx(trimStart : end);
        tSeg       = tSeg(trimStart : end);
        xSeg_full  = xSeg_full(trimStart : end);
        ySeg_full  = ySeg_full(trimStart : end);

        % Trim 2 ('both' only): remove the pick-up phase where the cursor is at
        % the inner-circle image before dragging outward.  The drag phase begins
        % at the first sample where the cursor is closer to any outer slot than
        % to any inner-circle image (i.e., the cursor has left image territory).
        if strcmp(reportType, 'both') && ~isempty(distStimMat)
            dStim = distStimMat(segMask, :);      % [nSegSamp × 6] inner-image distances
            dStim = dStim(trimStart : end, :);     % align with trimStart
            dSlot = distSeg(trimStart : end, :);   % outer-slot distances, same trim

            inInner   = min(dStim, [], 2) < min(dSlot, [], 2);
            lastInner = find(inInner, 1, 'last');
            leftDrag  = 1;
            if ~isempty(lastInner)
                leftDrag = lastInner + 1;
            end
            % Pick-up time: total time the cursor spent nearest to the specific
            % image that was eventually picked up and dragged to the slot.
            % The picked-up image is identified as the nearest inner-circle image
            % at lastInner (the final pick-up sample, just before drag onset).
            % Time is summed over all pick-up phase intervals where that specific
            % image is the closest inner-circle stimulus — analogous to dwellTime
            % for the drag phase, but measured in the inner-circle zone.
            if ~isempty(lastInner) && lastInner < length(tSeg)
                % Identify the picked-up image from the trajectory: the nearest
                % inner-circle image at the last pick-up sample (just before drag).
                [~, pickedUpImg]  = min(dStim(lastInner, :));
                [~, nearestInner] = min(dStim, [], 2);   % [nSampTrimmed × 1]
                dt_seg = diff(tSeg);                      % [nSampTrimmed-1 × 1]
                atPickedImg   = nearestInner(1:end-1) == pickedUpImg;
                inPickupPhase = inInner(1:end-1);
                pickupTime(iChoice)       = sum(dt_seg(atPickedImg & inPickupPhase));
                pickupConsistent(iChoice) = 1;   % valid pick-up phase detected
            else
                % No pick-up phase detected (cursor never entered inner zone).
                pickupTime(iChoice)       = 0;
                pickupConsistent(iChoice) = 0;
            end
            closestIdx = closestIdx(leftDrag : end);
            tSeg       = tSeg(leftDrag : end);

            % selSpeed over drag phase only (inner → outer slot, no return trip).
            xDrag = xSeg_full(leftDrag : end);
            yDrag = ySeg_full(leftDrag : end);
            if length(xDrag) > 1
                dragLen  = sum(sqrt(diff(xDrag).^2 + diff(yDrag).^2));
                dragTime = tSeg(end) - tSeg(1);
                if dragTime > 0
                    selSpeed(iChoice) = dragLen / dragTime;
                end
            end
        end

        if length(closestIdx) < 2
            continue;
        end

        % Time-weighted metrics: each sample k "owns" the interval [tSeg(k), tSeg(k+1)].
        % dt(k) is that interval's duration; closestIdx(k) is the nearest stimulus
        % throughout it. This accounts for variable frame rates across samples.
        dt     = diff(tSeg);    % [nSamp-1 × 1]
        totalT = sum(dt);
        if totalT == 0          % degenerate: all timestamps identical, cannot weight
            continue;
        end
        isChosen = closestIdx(1:end-1) == chosenSlots(iChoice);

        % Mind-change counting: only count switches among still-unchosen stimuli.
        % Samples where the cursor is closest to any previously chosen stimulus
        % are set to NaN and excluded from switch detection.
        closestIdx_eff = double(closestIdx);
        if iChoice > 1
            closestIdx_eff(ismember(closestIdx, chosenSlots(1:iChoice-1))) = NaN;
        end
        pairs      = [closestIdx_eff(1:end-1), closestIdx_eff(2:end)];
        validPairs = ~isnan(pairs(:,1)) & ~isnan(pairs(:,2));
        nChanges   = sum(pairs(validPairs, 1) ~= pairs(validPairs, 2));

        mindChanges(iChoice)    = nChanges;
        mindChanged(iChoice)    = double(nChanges > 0);
        timeInChosen(iChoice)   = sum(dt(isChosen)) / totalT;
        timeInOther(iChoice)    = 1 - timeInChosen(iChoice);
        closestAtClick(iChoice) = closestIdx(end);
        dwellTime(iChoice)      = sum(dt(isChosen));   % absolute seconds near chosen stim
    end

    % Pad partial-trial arrays to [1×nTrans] so that vertcat across trials
    % downstream always produces a consistent [nTrials×nTrans] matrix.
    % Unresponded positions are NaN and are ignored by omitnan operations.
    if nResp < nTrans
        pad            = NaN(1, nTrans - nResp);
        isCorrect      = [isCorrect,      pad];
        mindChanges    = [mindChanges,    pad];
        mindChanged    = [mindChanged,    pad];
        timeInChosen   = [timeInChosen,   pad];
        timeInOther    = [timeInOther,    pad];
        closestAtClick = [closestAtClick, pad];
        interClickTime = [interClickTime, pad];
        selSpeed       = [selSpeed,       pad];
        dwellTime      = [dwellTime,      pad];
        pickupTime        = [pickupTime,        pad];
        pickupConsistent  = [pickupConsistent,  pad];
    end

    % Accuracy-conditioned collapsed metrics (mean across transitions per trial).
    % NaN clicks (insufficient trajectory samples) and unknown-correctness clicks
    % (NaN isCorrect) are excluded. NaN == x is always false in MATLAB, so
    % corrMask / incorrMask naturally exclude those positions.
    corrMask   = (isCorrect == 1) & ~isnan(mindChanges);
    incorrMask = (isCorrect == 0) & ~isnan(mindChanges);
    corrMaskT  = (isCorrect == 1) & ~isnan(timeInChosen);
    incorrMaskT= (isCorrect == 0) & ~isnan(timeInChosen);

    mindChanges_corr    = NaN;
    mindChanges_incorr  = NaN;
    timeInChosen_corr   = NaN;
    timeInChosen_incorr = NaN;
    if any(corrMask),    mindChanges_corr    = mean(mindChanges(corrMask));    end
    if any(incorrMask),  mindChanges_incorr  = mean(mindChanges(incorrMask));  end
    if any(corrMaskT),   timeInChosen_corr   = mean(timeInChosen(corrMaskT));  end
    if any(incorrMaskT), timeInChosen_incorr = mean(timeInChosen(incorrMaskT));end

    corrMask_ict   = (isCorrect == 1) & ~isnan(interClickTime);
    incorrMask_ict = (isCorrect == 0) & ~isnan(interClickTime);
    corrMask_sp    = (isCorrect == 1) & ~isnan(selSpeed);
    incorrMask_sp  = (isCorrect == 0) & ~isnan(selSpeed);
    corrMask_dw    = (isCorrect == 1) & ~isnan(dwellTime);
    incorrMask_dw  = (isCorrect == 0) & ~isnan(dwellTime);
    corrMask_pk    = (isCorrect == 1) & ~isnan(pickupTime);
    incorrMask_pk  = (isCorrect == 0) & ~isnan(pickupTime);

    interClickTime_corr   = NaN; interClickTime_incorr = NaN;
    selSpeed_corr         = NaN; selSpeed_incorr        = NaN;
    dwellTime_corr        = NaN; dwellTime_incorr       = NaN;
    pickupTime_corr       = NaN; pickupTime_incorr      = NaN;
    if any(corrMask_ict),   interClickTime_corr   = mean(interClickTime(corrMask_ict));   end
    if any(incorrMask_ict), interClickTime_incorr = mean(interClickTime(incorrMask_ict)); end
    if any(corrMask_sp),    selSpeed_corr         = mean(selSpeed(corrMask_sp));          end
    if any(incorrMask_sp),  selSpeed_incorr       = mean(selSpeed(incorrMask_sp));        end
    if any(corrMask_dw),    dwellTime_corr        = mean(dwellTime(corrMask_dw));         end
    if any(incorrMask_dw),  dwellTime_incorr      = mean(dwellTime(incorrMask_dw));       end
    if any(corrMask_pk),    pickupTime_corr       = mean(pickupTime(corrMask_pk));        end
    if any(incorrMask_pk),  pickupTime_incorr     = mean(pickupTime(incorrMask_pk));      end

    mouse_data(iRep).nResp                = nResp;
    mouse_data(iRep).chosenSlots         = chosenSlots;
    mouse_data(iRep).clickTimes          = clickTimes;
    mouse_data(iRep).isCorrect           = isCorrect;
    mouse_data(iRep).mindChanges         = mindChanges;
    mouse_data(iRep).mindChanged         = mindChanged;
    mouse_data(iRep).timeInChosen        = timeInChosen;
    mouse_data(iRep).timeInOther         = timeInOther;
    mouse_data(iRep).closestAtClick      = closestAtClick;
    mouse_data(iRep).mindChanges_corr      = mindChanges_corr;
    mouse_data(iRep).mindChanges_incorr   = mindChanges_incorr;
    mouse_data(iRep).timeInChosen_corr    = timeInChosen_corr;
    mouse_data(iRep).timeInChosen_incorr  = timeInChosen_incorr;
    mouse_data(iRep).interClickTime       = interClickTime;
    mouse_data(iRep).selSpeed             = selSpeed;
    mouse_data(iRep).dwellTime            = dwellTime;
    mouse_data(iRep).pickupTime           = pickupTime;
    mouse_data(iRep).pickupConsistent     = pickupConsistent;
    mouse_data(iRep).interClickTime_corr  = interClickTime_corr;
    mouse_data(iRep).interClickTime_incorr= interClickTime_incorr;
    mouse_data(iRep).selSpeed_corr        = selSpeed_corr;
    mouse_data(iRep).selSpeed_incorr      = selSpeed_incorr;
    mouse_data(iRep).dwellTime_corr       = dwellTime_corr;
    mouse_data(iRep).dwellTime_incorr     = dwellTime_incorr;
    mouse_data(iRep).pickupTime_corr      = pickupTime_corr;
    mouse_data(iRep).pickupTime_incorr    = pickupTime_incorr;
end
end


function arr = parseMouseArray(cellStr)

    if ismissing(cellStr) || strlength(cellStr) == 0
        arr = [];
        return;
    end

    cellStr = char(cellStr);
    cellStr = regexprep(cellStr, '[\[\]]', '');
    cellStr = strrep(cellStr, ',', ' ');

    nums = regexp(cellStr, '[-+]?\d*\.?\d+', 'match');

    if isempty(nums)
        arr = [];
    else
        arr = str2double(nums);
    end
end


function s = pToSigStr(p)
    if isnan(p),       s = 'n/a';
    elseif p < 0.001,  s = '***';
    elseif p < 0.01,   s = '**';
    elseif p < 0.05,   s = '*';
    else,              s = 'ns';
    end
end

function drawBracket(ax, x1, x2, yB, tickLen, textOff, sigStr, clr, lw, fntSz)
% Draw a significance bracket from x1 to x2 at height yB.
    plot(ax, [x1, x2], [yB, yB],           '-',  'Color', clr, 'LineWidth', lw, 'HandleVisibility', 'off');
    plot(ax, [x1, x1], [yB - tickLen, yB], '-',  'Color', clr, 'LineWidth', lw, 'HandleVisibility', 'off');
    plot(ax, [x2, x2], [yB - tickLen, yB], '-',  'Color', clr, 'LineWidth', lw, 'HandleVisibility', 'off');
    text(ax, (x1 + x2) / 2, yB + textOff, sigStr, ...
        'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', ...
        'FontSize', fntSz, 'FontName', 'Arial', 'Color', clr);
end


%% Plotting the mouse trajectory along with time information for examplar participant





